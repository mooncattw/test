...
local function buildMobileButtons()
    ...
    -- ===== ABYSS/ZEN STYLE =====
    local QS = 58          -- button size px
    local QG = 8           -- gap px
    local QR = 13          -- corner radius
    local Q_OFF        = Color3.fromRGB(15, 20, 34)
    local Q_ON         = Color3.fromRGB(79, 142, 255)
    local Q_BORDER     = Color3.fromRGB(34, 50, 78)
    local Q_BORDER_ON  = Color3.fromRGB(110, 170, 255)
    local Q_TEXT       = Color3.fromRGB(230, 237, 255)
    ...
    mbGroup.Position = UDim2.new(1, -QW - 18, 1, -(QH + 24))
    mbGroup.BackgroundTransparency = 1
    mbGroup.BorderSizePixel = 0
    mbGroup.Active = true
    mbGroup.ZIndex = 100

    local dockLine = Instance.new("Frame", mobGui)
    dockLine.Size = UDim2.new(0, 96, 0, 1)
    dockLine.Position = UDim2.new(1, -(QW + 18 + 96), 1, -(QH + 20))
    dockLine.BackgroundColor3 = Color3.fromRGB(79, 142, 255)
    ...
    local _, refDrop = makeMobileBtn("DROP", 0, 0, false, function()
        runDrop()
    end)
    mobBtnRefs["drop"] = refDrop

    local _, refAutoLeft = makeMobileBtn("AUTO L", 1, 0, true, function(on)
        if on then
    ...
    local _, refAutoBat = makeMobileBtn("BAT AIM", 0, 1, true, function(on)
        if on then
    ...
    local _, refAutoRight = makeMobileBtn("AUTO R", 1, 1, true, function(on)
        if on then
            if autoLeftEnabled then autoLeftEnabled=false; stopAutoLeft(); if autoLeftSetVisual then autoLeftSetVisual(false) end; if mobBtnRefs.autoLeft then mobBtnRefs.autoLeft(false) end end
            if autoBatEnabled then stopBatAimbot(); if autoBatSetVisual then autoBatSetVisual(false) end; if mobBtnRefs.autoBat then mobBtnRefs.autoBat(false) end end
    ...
    local _, refTP = makeMobileBtn("TP", 0, 2, false, function()
        runTPFloor()
    end)
    mobBtnRefs["tpDown"] = refTP

    local _, refCarry = makeMobileBtn("CARRY", 1, 2, true, function(on)
        toggleCarryMode()
        if mobBtnRefs.lagger then mobBtnRefs.lagger(laggerModeEnabled) end
        saveConfig()
    end)
    ...
    local _, refLagger = makeMobileBtn("LAGGER", 0, 3, true, function(on)
        toggleLaggerMode()
        if mobBtnRefs.lagger then mobBtnRefs.lagger(laggerModeEnabled) end
        if mobBtnRefs.carrySpeed then mobBtnRefs.carrySpeed(carrySpeedActive) end
        saveConfig()
    ...
    local _, refReset = makeMobileBtn("RESET", 1, 3, false, function()
        cursedInstaReset()
    end)
...
local C={
    bg=Color3.fromRGB(8,10,18), bgDark=Color3.fromRGB(4,6,12), row=Color3.fromRGB(16,20,32),
    input=Color3.fromRGB(15,19,30), blue=Color3.fromRGB(79,142,255), blueDim=Color3.fromRGB(52,78,132),
    blueDark=Color3.fromRGB(14,20,34), text=Color3.fromRGB(245,247,255), textDim=Color3.fromRGB(170,182,210),
    textMuted=Color3.fromRGB(108,119,144), white=Color3.fromRGB(255,255,255), divider=Color3.fromRGB(34,50,78),
...
;(function()
    ...
    ML.Position=UDim2.new(0,14,0,32); ML.Size=UDim2.new(0,220,0,14); ML.BackgroundTransparency=1
    ML.Text="moon hub • dark ui • blue accents"; ML.TextColor3=C.textDim; ML.TextSize=10; ML.Font=Enum.Font.GothamBold
    ...
    HSep.BackgroundTransparency=0.55; HSep.BorderSizePixel=0; HSep.Parent=Inner; HSep.ZIndex=2
