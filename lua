...
-- ============================================================
-- NatAov Hub | Blox Fruits Auto Script
-- Tác giả  : Dev-AnhTuanSiTink 
-- Phiên bản: Blox Fruits Main
-- Mô tả    : Hub tự động hóa toàn diện cho game Blox Fruits
--   Auto Farm Level / Quest / Boss / Material / Chest / Berry
--   Sea Event / Raid / Prehistoric / Mirage / Race V4
--   Teleport / Shop / ESP / Combat / Fishing / CDK Quests
-- Discord   : https://discord.gg/UefGEBySJ8
-- ============================================================

do

-- ============================================================
-- [BLOCK 1] KHỞI TẠO BIẾN TOÀN CỤC
-- Khai báo service, tham chiếu nhân vật và biến trạng thái
-- ply/plr/Root: tham chiếu Player và Character
-- replicated: ReplicatedStorage
-- TW, Lighting, Enemies, vim1/vim2, RunSer, Stats: service
-- ============================================================
  ply = game.Players
  plr = ply.LocalPlayer
...
-- shouldTween: đang tween di chuyển
-- SoulGuitar, KenTest, debug, Brazier1/2/3: cờ tính năng
-- Sec=0.1: delay mặc định
-- Num_self=25: ngưỡng HP% kích hoạt Safe Mode
  BringConnections = {}
  BossList = {}
  MaterialList = {}
  NPCList = {}
...
-- ============================================================
-- [BLOCK 2] CHỜ GAME TẢI XONG
-- Chờ UI Loading và game:IsLoaded() = true
-- ============================================================
repeat local start = plr.PlayerGui:WaitForChild("Main"):WaitForChild("Loading") and game:IsLoaded() wait() until start

-- Xác định biển hiện tại:
-- World1=First Sea, World2=Second Sea, World3=Third Sea
...
-- Marines(): chuyển sang phe Hải quân
-- Pirates(): chuyển sang phe Cướp biển
...
-- Danh sách boss và nguyên liệu theo từng biển
-- BossList, MaterialList được dùng trong Dropdown UI
...
-- Bảng phụ trợ: DungeonTables, RenMon, CursedTables,
-- Past, BartMon, CitizenTable, Human_v3_Mob, AllBoats,
-- mastery1/2: nhóm mob đặc biệt theo chức năng
-- PosMsList: vị trí CFrame spawn mob World3
...
-- Remote Functions hay dùng: RF/JobsRemoteFunction, RF/Craft
local Remotes = {
...
-- ============================================================
-- [BLOCK 3] HÀM TRANG BỊ VŨ KHÍ
-- EquipWeapon(text): equip tool theo tên từ Backpack
-- weaponSc(weapon): equip tool theo ToolTip (Melee/Sword/Gun/Fruit)
-- ============================================================
...
-- ============================================================
-- [BLOCK 4] MODULE ATTACK - Các hàm tấn công mob
-- Attack.Alive(model)   : Kiểm tra mob còn sống
-- Attack.Dist(m,d)      : Khoảng cách <= d
-- Attack.DistH(m,d)     : Khoảng cách > d
-- Attack.Kill(m,S)      : TP+tấn công, delay 0.5s xoay quanh
-- Attack.Kill2(m,S)     : TP+tấn công, delay 0.1s (nhanh hơn)
-- Attack.KillSea(m,S)   : TP lên cao Y+50/400 cho mob biển
-- Attack.Sword(m,S)     : Tấn công bằng Sword
-- Attack.Mas(m,S)       : Fruit skill khi HP thấp, Melee khi cao
-- Attack.Masgun(m,S)    : Gun skill khi HP thấp, Melee khi cao
-- ============================================================
local Attack = {}
...
-- ============================================================
-- [BLOCK 5] PHÂN PHỐI ĐIỂM STAT
-- statsSetings(Num,val): Gửi AddPoint cho Melee/Defense/Sword/Gun/Devil
-- ============================================================
...
-- ============================================================
-- [BLOCK 6] KÉO MOB - BringEnemy(Mon)
-- Tự động tìm mob gần nhất nếu không truyền tham số
-- SimulationRadius=math.huge để lấy network ownership
-- Tạo BodyVelocity giữ mob đứng yên (Velocity=zero)
-- Kéo mob cùng tên trong bán kính 3000 về điểm tập trung
-- ============================================================
...
-- ============================================================
-- [BLOCK 7] SỬ DỤNG KỸ NĂNG - Useskills(weapon, skill)
-- Trang bị weapon rồi gửi key event giả (VirtualInputManager)
-- weapon: Melee/Sword/Blox Fruit/Gun | skill: Z/X/C/V/Y
-- ============================================================
...
-- ============================================================
-- [BLOCK 8] HOOK __NAMECALL - Redirect hướng tấn công
-- setreadonly(gg, false) mở metatable để can thiệp
-- Khi FireServer gọi trên RemoteEvent + các flag farm bật:
--   Ghi đè args[2] = MousePos để aim tự động
-- ============================================================
...
-- ============================================================
-- [BLOCK 9] TÌM MOB - GetConnectionEnemies(a)
-- Tìm trong ReplicatedStorage trước, sau đó workspace.Enemies
-- Hỗ trợ tìm theo string hoặc table tên
-- ============================================================
...
-- ============================================================
-- [BLOCK 10] GIẢM TẢI CPU - LowCpu()
-- Tắt shadow/fog/bloom/DOF, giảm quality Level01
-- Đổi vật liệu Plastic, xóa Particle/Trail/Fire/Smoke
-- ============================================================
LowCpu = function()
...
-- ============================================================
-- [BLOCK 11] CÁC HÀM KIỂM TRA TRẠNG THÁI
-- CheckF/CheckBoat/CheckEnemiesBoat/CheckPirateGrandBrigade
-- CheckShark/CheckTerrorShark/CheckPiranha
-- CheckFishCrew/CheckHauntedCrew/CheckSeaBeast/CheckLeviathan
-- ============================================================
CheckF = function()
...
-- ============================================================
-- [BLOCK 12] QUẢN LÝ TRÁI CÂY
-- UpdStFruit: lưu kho | collectFruits: kéo về
-- Getmoon: texture mặt trăng | DropFruits: drop ra
-- GetBP/GetIn/GetM/GetWP: kiểm tra inventory
-- ============================================================
UpdStFruit = function()
...
-- ============================================================
-- [BLOCK 13] INFINITY ABILITY - getInfinity_Ability(Method,Var)
-- Soru: reset cooldown liên tục
-- Energy: giữ Energy.Value không giảm
-- Observation: VisionRadius = math.huge
-- ============================================================
...
-- ============================================================
-- [BLOCK 14] SERVER HOP - Hop()
-- Gọi __ServerBrowser, tìm server < 12 người và teleport
-- ============================================================
Hop = function()
  pcall(function()
...
-- ============================================================
-- [BLOCK 15] NOCLIP BLOCK + TWEEN MOVEMENT SYSTEM
-- Part "Rip_Indra" vô hình làm anchor di chuyển tween
-- Task 1: Đồng bộ OnFarm với shouldTween
-- Task 2: Nhân vật đi theo block khi OnFarm=true
--   CanCollide=false khi farm, true khi dừng
-- ============================================================
...
-- Cấu hình: TweenSpeed=350, BypassTeleport=true
-- newdao=vị trí Submerged Island, cframenpc=NPC tàu ngầm
local Settings = {
    ["Tween Speed"] = 350,
    ["Bypass Teleport"] = getgenv().bypassTP or false,
    ["Up Y"] = false,
...
-- ============================================================
-- [BLOCK 16] HÀM TIỆN ÍCH TELEPORT
-- Convert_CFrame: Vector3/Model/Part -> CFrame
-- GetDistance: tính khoảng cách (bỏ Y nếu NO_Y=true)
-- InArea: xác định khu vực qua _WorldOrigin.Locations
-- GetSpawnPoint: tìm PlayerSpawn gần vị trí x
-- CheckLegendaryItems: kiểm tra Chalice/FoD/v.v...
-- WaitForHumanoid: chờ Humanoid, timeout 5s
-- checkinventory: kiểm tra item trong inventory
-- getdis: khoảng cách 2D (bỏ trục Y)
-- ============================================================
...
-- ============================================================
-- [BLOCK 17] BYPASS TELEPORT SYSTEM
-- CanBypassTeleport: không ở Dimension/Sub/Cave + không có item quý
-- GetBypassCFrame: tìm spawn trung gian gần đích nhất
-- BypassTP: SetLastSpawnPoint + PivotTo spawn trung gian
-- ============================================================
...
-- ============================================================
-- [BLOCK 18] VÀO KHU VỰC ĐẶC BIỆT
-- totopofgreattree: TP lên đỉnh Great Tree (Race V4)
-- requestentrance(pos): xác định khu vực đặc biệt gần đích
--   Sea1: Sky, Underwater | Sea2: Swan, Cursed Ship
--   Sea3: Hydra, Temple, Great Tree
-- ============================================================
...
-- ============================================================
-- [BLOCK 19] HÀM TELEPORT CHÍNH
-- _tp(target): tween block Part tới đích, nhân vật đi theo
--   1. Bypass TP nếu khoảng cách > 3500
--   2. requestentrance vào khu vực đặc biệt
--   3. Xử lý Submerged Island qua NPC tàu ngầm
--   4. Tween với tốc độ distance/300
-- old_tp: teleport trực tiếp không tween
-- notween: alias cho old_tp
-- TeleportToTarget: wrapper của _tp
-- BTP: bypass cưỡng bức (reset health=0 + set CFrame)
-- ============================================================
_tp = function(target)
    local gg
...
-- ============================================================
-- [BLOCK 21] BOSS QUEST - QuestB()
-- Map _G.FindBoss -> bMon, Qname, Qdata, PosQBoss, PosB
-- QuestBeta() trả về table [0..5] đầy đủ dữ liệu
-- ============================================================
QuestB = function()
				if World1 then
...
-- ============================================================
-- [BLOCK 22] HỆ THỐNG QUEST TỰ ĐỘNG
-- CheckSea: kiểm tra PlaceId khớp biển
-- GetQuestPointFromNPC: tìm vị trí NPC trong workspace/replicated
-- GetQuests: tìm quest phù hợp level (loại trừ Marine/Bartilo)
-- MaterialMon: map nguyên liệu -> mob và vị trí farm
-- QuestNeta: {Mob, ID, NameQuest, LevelReq, Mob, QuestPos}
-- ============================================================
...
-- ============================================================
-- [BLOCK 23] KHỞI TẠO UI - redzlib v5
-- Load từ GitHub, Title="NatAov Hub : Blox Fruit"
-- SaveFolder="oknaiget.json", Minimizer=LeftControl
-- ============================================================
-- ============================================================
-- [BLOCK 23] KHỞI TẠO UI - ProxyLib (Thay thế redzlib v5)
-- ============================================================
...
local MainWindow = Window:CreateWindow({
    ...
    Words = {
        ...
        FloatButton = { Shape = "Circle", Color = "Black", Size = 60, Icon = "rbxassetid://120488231660846" },

        ConfigPanel = {
            Enabled = true, Acrylic = true, Theme = true,
            Fps = true, Ping = true, Profile = true,
            HideNotify = true, Language = true, BackgroundImage = true,
        },
    })


-- ============================================================
-- [BLOCK 24] ĐỊNH NGHĨA 14 TAB CHÍNH
-- Info/Main/Settings/Fish/Quests/SeaEvent/Race
-- Prehistoric/Esp/Raids/Combat/Travel/Shop/Misc
-- ============================================================
local Tabs = {
    Discord        = MainWindow:CreateTab({ Title = "Discord",   Subtitle = "NatAovHub",  Icon = "rbxassetid://73132811772878" }),
    Info        = MainWindow:CreateTab({ Title = "Status Server",   Subtitle = "Server Info",  Icon = "rbxassetid://7040410130" }),
    Main        = MainWindow:CreateTab({ Title = "Farming",         Subtitle = "Auto Farm",    Icon = "rbxassetid://7733960981" }),
    Settings    = MainWindow:CreateTab({ Title = "Settings",        Subtitle = "Configure",    Icon = "rbxassetid://7734053495" }),
    Fish        = MainWindow:CreateTab({ Title = "Fishing",         Subtitle = "Auto Fish",    Icon = "rbxassetid://127664059821666" }),
    Quests      = MainWindow:CreateTab({ Title = "Quests & Items",  Subtitle = "Item Farm",    Icon = "rbxassetid://13075622619" }),
    SeaEvent    = MainWindow:CreateTab({ Title = "Sea Event",       Subtitle = "Events",       Icon = "rbxassetid://10747376931" }),
    Race        = MainWindow:CreateTab({ Title = "Mirage & Race",   Subtitle = "Race V4",      Icon = "rbxassetid://11162889532" }),
    Prehistoric = MainWindow:CreateTab({ Title = "Volcano Event",   Subtitle = "Prehistoric",  Icon = "rbxassetid://10734897956" }),
    Esp         = MainWindow:CreateTab({ Title = "Stats & ESP",     Subtitle = "Esp",          Icon = "rbxassetid://7040410130" }),
    Raids       = MainWindow:CreateTab({ Title = "Fruit & Raid",    Subtitle = "Raid",         Icon = "rbxassetid://11155986081" }),
    Combat      = MainWindow:CreateTab({ Title = "Local Player",    Subtitle = "Combat",       Icon = "rbxassetid://13075651575" }),
    Travel      = MainWindow:CreateTab({ Title = "Teleport",        Subtitle = "Travel",       Icon = "rbxassetid://10734906975" }),
    Shop        = MainWindow:CreateTab({ Title = "Shopping",        Subtitle = "Shop",         Icon = "rbxassetid://6031265976" }),
    Misc        = MainWindow:CreateTab({ Title = "Miscellaneous",   Subtitle = "Misc",         Icon = "rbxassetid://10709783577" }),
}
Tabs.Discord:Select()

-- ============================================================
-- [BLOCK 25] TAB INFO - THÔNG TIN SERVER
-- ============================================================

Tabs.Discord:CreateSection({ Text = "Information", Icon = "rbxassetid://0" })

Tabs.Discord:CreateDiscordInvite({
	Title = "NatAov Hub | Community",
	Description = "A community for NatAov Hub Users - official scripts, updates, and suport in one place.",
	Banner = "rbxassetid://110167557685296", 
	Icon = "rbxassetid://120488231660846",
	Link = "https://discord.gg/UefGEBySJ8",
    Button = "Join Server",
...
-- 🛠️ DEBUG UI
-- ==============================================================================
Tabs.Discord:CreateSection({ Text = "Debug Farm" })

local DebugParagraph = Tabs.Discord:CreateParagraph({
    Title       = "Các Toggle Đang Bật",
    Description = "Không có toggle nào đang bật.",
})

-- ==============================================================================
-- ⚙️ DEBUG SYSTEM v2 — FIX DEFAULT / AUTOLOAD KHÔNG GỌI CALLBACK
-- ==============================================================================
local ToggleObjects = {}  -- { Obj, Tab, Name }
local ActiveToggles = {}  -- { [TabName] = { [ToggleName] = true } }
local PrevStates    = {}  -- snapshot trạng thái kỳ trước để tránh SetDescription thừa

-- ─── Đọc trạng thái thực tế của một ToggleObj ────────────────────────────────
local function GetToggleState(obj)
    if obj == nil then return false end
    -- ProxyLib dùng method :Get() thay vì property .Value/.State
    if type(obj.Get) == "function" then
        local ok, val = pcall(obj.Get, obj)
        if ok and type(val) == "boolean" then return val end
    end
    return false
end

-- ─── Rebuild ActiveToggles từ snapshot thực tế rồi cập nhật UI ───────────────
local function UpdateDebug()
    -- BƯỚC 1: Quét toàn bộ ToggleObjects → rebuild ActiveToggles
    local newActive = {}

    for _, item in ipairs(ToggleObjects) do
        if GetToggleState(item.Obj) then
            newActive[item.Tab] = newActive[item.Tab] or {}
            newActive[item.Tab][item.Name] = true
        end
    end

    -- BƯỚC 2: So sánh với snapshot kỳ trước; chỉ gọi SetDescription khi có thay đổi
    -- (tránh spam update mỗi frame)
    local changed = false

    -- Kiểm tra có tab/toggle mới bật hoặc tắt không
    for tabName, toggles in pairs(newActive) do
        if not PrevStates[tabName] then
            changed = true; break
        end
        for tName in pairs(toggles) do
            if not PrevStates[tabName][tName] then
                changed = true; break
            end
        end
    ...
    if not changed then
        for tabName, toggles in pairs(PrevStates) do
            if not newActive[tabName] then
                changed = true; break
            end
            for tName in pairs(toggles) do
                if not newActive[tabName][tName] then
                    changed = true; break
                end
            end
            if changed then break end
    ...
    if not changed then return end  -- Không có gì thay đổi → bỏ qua

    -- BƯỚC 3: Cập nhật snapshot
    ActiveToggles = newActive
    PrevStates    = {}
    for k, v in pairs(newActive) do
        PrevStates[k] = {}
    ...
    -- BƯỚC 4: Tạo chuỗi hiển thị
    local Lines    = {}
    local TabNames = {}

    for TabName in pairs(ActiveToggles) do
        table.insert(TabNames, TabName)
    end
    table.sort(TabNames)

    for _, TabName in ipairs(TabNames) do
        local Toggles = ActiveToggles[TabName]
        if next(Toggles) then
            table.insert(Lines, "📂 " .. TabName)
    ...
    local newText = (#Lines == 0)
        and "Không có toggle nào đang bật."
        or  table.concat(Lines, "\n")

    DebugParagraph:SetDescription(newText)
end

-- ─── Hook một Tab để thu thập ToggleObj sau khi tạo ──────────────────────────
local function HookTab(Tab, TabName)
    if not Tab or not Tab.CreateToggle then return end

    local OldCreateToggle = Tab.CreateToggle

    Tab.CreateToggle = function(self, ToggleData)
        local OldCallback   = ToggleData.Callback
        local ToggleName    = ToggleData.Title or ToggleData.Name or "Unknown Toggle"

        -- Callback vẫn giữ để cập nhật tức thì khi người dùng click
        ToggleData.Callback = function(Value)
            UpdateDebug()
            if OldCallback then
                task.spawn(OldCallback, Value)
            end
        end

        local ToggleObj = OldCreateToggle(self, ToggleData)
...
-- 🔗 HOOK HẾT ĐỐNG TAB
-- ==============================================================================
HookTab(Tabs.Discord,    "Discord")
HookTab(Tabs.Info,       "Status Server")
HookTab(Tabs.Main,       "Farming")
HookTab(Tabs.Settings,   "Settings")
HookTab(Tabs.Fish,       "Fishing")
HookTab(Tabs.Quests,     "Quests & Items")
HookTab(Tabs.SeaEvent,   "Sea Event")
HookTab(Tabs.Race,       "Mirage & Race")
HookTab(Tabs.Prehistoric,"Volcano Event")
HookTab(Tabs.Esp,        "Stats & ESP")
HookTab(Tabs.Raids,      "Fruit & Raid")
HookTab(Tabs.Combat,     "Local Player")
HookTab(Tabs.Travel,     "Teleport")
HookTab(Tabs.Shop,       "Shopping")
HookTab(Tabs.Misc,       "Miscellaneous")




Tabs.Info:CreateSection({ Text = "Status Server", Icon = "rbxassetid://0" })

local TimeZone = Tabs.Info:CreateParagraph({ Title = "Time Zone", Description = "" })


-- UpdateOS(): giờ địa phương + country code mỗi giây
function UpdateOS()
    ...
    TimeZone:SetDescription(datetime.." - "..timezone.." [ " .. code .. " ]")
end
...
local GameTime = Tabs.Info:CreateParagraph({ Title = "Game Time", Description = "" })


-- UpdateGameTime(): giờ/phút/giây in-game từ DistributedGameTime
function UpdateGameTime()
    ...
    GameTime:SetDescription(Hour.." Hour (h) "..Minute.." Minute (m) "..Second.." Second (s)")
end
...
-- Theo dõi island đặc biệt: Mirage, Kitsune, Prehistoric, Frozen

local MirageCheck = Tabs.Info:CreateParagraph({ Title = "Mirage Island", Description = "Status: " })
...
local KitsuneCheck = Tabs.Info:CreateParagraph({ Title = "Kitsune Island", Description = "Status: " })
...
spawn(function()
    while task.wait(1) do
        ...
        if currentStatus ~= previousKitsuneStatus then
            KitsuneCheck:SetDescription('Status: ' .. currentStatus)
            previousKitsuneStatus = currentStatus
        end
    end
end)

local PrehistoricCheck = Tabs.Info:CreateParagraph({ Title = "Prehistoric Island", Description = "Status: " })
...
task.spawn(function()
    while task.wait(1) do
        ...
        if currentStatus ~= previousPrehistoricStatus then
            PrehistoricCheck:SetDescription("Status: " .. currentStatus)
            previousPrehistoricStatus = currentStatus
        end
    end
end)

local FrozenCheck = Tabs.Info:CreateParagraph({ Title = "Frozen Dimension", Description = "Status: " })
...
spawn(function()
    while wait(1) do
        ...
        if currentStatus ~= previousFrozenStatus then
            FrozenCheck:SetDescription('Status: ' .. currentStatus)
...
-- Theo dõi: Cake Prince, Full Moon phase, Legendary Sword, Bone count

local CakePrinceStatus = Tabs.Info:CreateParagraph({ Title = "Cake Prince", Description = "" })
...
spawn(function()
    while wait(1) do
        ...
        CakePrinceStatus:SetDescription(killStatus)
    end
end)

local RipIndraCheck = Tabs.Info:CreateParagraph({ Title = "Rip Indra", Description = "Status: " })
...
spawn(function()
    while wait(1) do
        ...
        if currentStatus ~= previousRipStatus then
            RipIndraCheck:SetDescription("Status: " .. currentStatus)
            previousRipStatus = currentStatus
        end
    end
end)

local DoughKingCheck = Tabs.Info:CreateParagraph({ Title = "Dough King", Description = "Status: " })
...
spawn(function()
    while wait(1) do
        ...
        if currentStatus ~= previousDoughStatus then
            DoughKingCheck:SetDescription("Status: " .. currentStatus)
            previousDoughStatus = currentStatus
        end
    end
end)

local Pullever = Tabs.Info:CreateParagraph({
    Title = "Pull Level",
    Description = ""
})
...
local FullMoonCheck = Tabs.Info:CreateParagraph({ Title = "Full Moon", Description = "" })
...
task.spawn(function()
    while task.wait(1) do
        ...
        FullMoonCheck:SetDescription(moonStatus)
    end
end)

local LegendarySwordCheck = Tabs.Info:CreateParagraph({ Title = "Legendary Sword", Description = "Status: " })
...
spawn(function()
    while wait(1) do
        ...
        LegendarySwordCheck:SetDescription(swordStatus)
    end
end)

local BoneCount = Tabs.Info:CreateParagraph({ Title = "Bone", Description = "" })

spawn(function()
    while wait(1) do
        local bones = game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Bones", "Check")
        BoneCount:SetDescription("You Have: " .. tostring(bones) .. " Bones")
    end
end)

local PlaceIdParagraph = Tabs.Info:CreateParagraph({
    Title = "Place ID",
    Description = ""
})
...
spawn(function()
    while true do
        pcall(function()
            PlaceIdParagraph:SetDescription("ID: " .. tostring(game.PlaceId))
        end)
        wait(1)
    end
end)
...
-- ============================================================
-- [BLOCK 26] TAB MAIN - FARMING
-- WeaponDropdown: Melee/Sword/Blox Fruit/Gun
-- Cập nhật _G.SelectWeapon theo ToolTip mỗi 0.5s
-- UI Scale: Small(0.8)/Normal(1.0)/Big(1.2)
-- ============================================================

WeaponDropdown = Tabs.Main:CreateDropdown({
    Title = "Select Weapon",
...
Tabs.Main:CreateSection({ Text = "Farming", Icon = "rbxassetid://0" })


-- ============================================================
-- [BLOCK 27] AUTO FARM LEVEL
-- Level>=2600+chưa ở Sub: TP tới NPC tàu ngầm -> vào Sub
-- Khi ở Sub hoặc level<2600:
--   Lấy quest, nhận quest tại NPC, tìm mob tấn công
--   Nếu không thấy mob: TP tới spawn point của mob
-- ============================================================

FarmLevel = Tabs.Main:CreateToggle({
...
-- ============================================================
-- [BLOCK 28] AUTO FARM NEAREST
-- Tấn công mob gần nhất trong workspace.Enemies
-- ============================================================

ClosetMons = Tabs.Main:CreateToggle({
...
-- ============================================================
-- [BLOCK 29] AUTO FACTORY RAID
-- Core tồn tại: tấn công liên tục
-- Không có Core: TP vào Factory chờ spawn
-- ============================================================

FactoryRaids = Tabs.Main:CreateToggle({
...
-- ============================================================
-- [BLOCK 30] AUTO PIRATE/CASTLE RAID
-- Trong Castle bán kính 500: tấn công mob bán kính 2000
-- Ngoài: tìm mob raid trong ReplicatedStorage -> TP vào
-- ============================================================

CastleRaids = Tabs.Main:CreateToggle({
...
-- ============================================================
-- [BLOCK 31] AUTO FARM ECTOPLASM
-- Farm Ship mob tại Cursed Ship để lấy Ectoplasm
-- ============================================================

Ecto = Tabs.Main:CreateToggle({
...
-- ============================================================
-- [BLOCK 32] AUTO FARM CHEST (Tween)
-- "_ChestTagged" CollectionService -> tìm rương gần nhất -> TP
-- ============================================================

Tabs.Main:CreateSection({ Text = "Chest", Icon = "rbxassetid://0" })

ChestTW = Tabs.Main:CreateToggle({
...
-- ============================================================
-- [BLOCK 33] AUTO CHEST BYPASS
-- TouchInterest detect -> sort khoảng cách -> TP HRP vào rương
-- Tự động khởi động lại sau khi nhân vật hồi sinh
-- ============================================================

ChestBP = Tabs.Main:CreateToggle({
...
-- StopItems: tắt chest farm khi nhặt God's Chalice/Sweet Chalice/FoD

StopI = Tabs.Main:CreateToggle({
Title = "Stop Items", 
Description = "", 
Default = true,
Callback = function(Value)
...
-- ============================================================
-- [BLOCK 34] AUTO FARM BERRY
-- "BerryBush" CollectionService -> fireproximityprompt hái berry
-- ============================================================

Tabs.Main:CreateSection({ Text = "Collect Berry", Icon = "rbxassetid://0" })

Berry = Tabs.Main:CreateToggle({
...
-- ============================================================
-- [BLOCK 35] AUTO FARM BERRY + HOP
-- Không có berry: server hop qua Roblox Games API
-- ============================================================

BerryH = Tabs.Main:CreateToggle({
...
-- ============================================================
-- [BLOCK 36] FARM MOB THEO CHỌN
-- Select Mob dropdown theo biển (World1/2/3)
-- Auto Kill Mob: Heartbeat loop tấn công mob đã chọn
-- ============================================================

Tabs.Main:CreateSection({ Text = "Farm Mob", Icon = "rbxassetid://0" })
if World1 then
...
-- ============================================================
-- [BLOCK 37] AUTO FARM ALL ISLAND
-- Sea1/2/3 Islands: bảng {CFrame, Mobs} cho từng đảo
-- Select Island dropdown + tấn công mob của đảo đó
-- ============================================================

Tabs.Main:CreateSection({ Text = "Farm All Island", Icon = "rbxassetid://0" })
...
-- ============================================================
-- [BLOCK 38] FARM ELITE HUNTER
-- Auto Farm Elite: nhận quest, tìm Diablo/Urban/Deandre
-- Auto Farm Elite + Hop: hop server khi hết boss
-- HopServer(): Roblox API -> tìm server có chỗ -> TP
-- ============================================================

Tabs.Main:CreateSection({ Text = "Farm Elite Hunter", Icon = "rbxassetid://0" })

local Process = Tabs.Main:CreateParagraph({ Title = "Elites Process", Description = "" })
spawn(function()
    while wait(Sec) do
        pcall(function()    
            Process:SetDescription("Elite Progress : " .. replicated.Remotes.CommF_:InvokeServer("EliteHunter", "Progress"))
        end)
    end
end)

local EliteHunter = Tabs.Main:CreateParagraph({ Title = "Elite Spawn", Description = "Status: " })
spawn(function()
    ...
    while wait(1) do
        ...
        if currentStatus ~= previousStatus then
            EliteHunter:SetDescription("Status: " .. currentStatus .. " | Killed: " .. progress)
            previousStatus = currentStatus
        end
    end
end)

EliteQ = Tabs.Main:CreateToggle({
...
-- ============================================================
-- [BLOCK 39] FARM RIP INDRA
-- Kiểm tra Dark Dagger/Valkyrie rồi requestEntrance vào khu vực
-- ============================================================

Tabs.Main:CreateSection({ Text = "Farm Rip Indra", Icon = "rbxassetid://0" })
...
-- ============================================================
-- [BLOCK 40] AUTO UNLOCKED HAKI (Boat Castle)
-- AuraSkin: equip aura theo ID
-- VaildColor: kiểm tra màu Lime green
-- HakiCalculate: map BrickColor -> tên Haki
-- ============================================================
...
-- ============================================================
-- [BLOCK 41] FARMING CAKE ISLAND
-- Hiển thị số Cake Prince đã kill (500 - remaining)
-- Auto Farm Cake Prince: Quest -> farm mob -> tấn công boss
-- Auto Summon Cake Prince: CakePrinceSpawner khi đủ điều kiện
-- ============================================================

Tabs.Main:CreateSection({ Text = "Farming Cake", Icon = "rbxassetid://0" })

local MobKilled = Tabs.Main:CreateParagraph({ Title = "Cake Princes", Description = "" })
...
spawn(function()
    while wait(0.2) do
        pcall(function()
            ...
            if Killed then
                MobKilled:SetDescription("Killed : " .. (500 - tonumber(Killed) or 0))
            end
        end)
    end
end)

Cake = Tabs.Main:CreateToggle({
...
-- ============================================================
-- [BLOCK 42] GRAVESTONE EVENTS
-- Auto Hyena + Hallow: tấn công Hyena, mua/dùng Hallow Essence
-- Auto Random Bones: mua random Bone box liên tục
-- Auto Try Luck / Pray: gravestoneEvent(1) / gravestoneEvent(2)
-- ============================================================
end)
...
Tabs.Main:CreateSection({ Text = "Farming Bone", Icon = "rbxassetid://0" })

local CheckingBone = Tabs.Main:CreateParagraph({ Title = "Bones", Description = "" })
spawn(function()
    while wait(0.2) do
        pcall(function()
...
RanBone = Tabs.Main:CreateToggle({
...
-- ============================================================
-- [BLOCK 43] TYRANT OF THE SKIES BOSS
-- Check_Eye(): đếm mắt sáng (4/4 summon boss)
-- Auto Farm Boss TOTS: tấn công Tyrant hoặc mob xung quanh
-- Auto Summon Boss: đi qua PhaBinhPoints dùng kỹ năng
-- ============================================================

Tabs.Main:CreateSection({ Text = "Tyrant of the Skies", Icon = "rbxassetid://0" })

local TyrantStatus = Tabs.Main:CreateParagraph({ Title = "Boss Spawn", Description = "" })
spawn(function()
    pcall(function()
        while wait(1) do
...
local EyeStatus = Tabs.Main:CreateParagraph({ Title = "Check Status Eyes", Description = "" })
...
task.spawn(function()
    ...
    while task.wait(1) do
        ...
        EyeStatus:SetDescription("Eyes: " .. current .. "/4")
...
FarmTyrant = Tabs.Main:CreateToggle({
...
FarmPhaBinh = Tabs.Main:CreateToggle({
...
-- ============================================================
-- [BLOCK 44] FARM MATERIAL (Nguyên liệu)
-- MaterialMon(): map SelectMaterial -> MMon và MPos
-- Auto Farm Materials: TP tới MPos, tấn công mob trong MMon
-- Xử lý cả EnemySpawns nếu không thấy mob
-- ============================================================

Tabs.Main:CreateSection({ Text = "Farm Material", Icon = "rbxassetid://0" })

Test = Tabs.Main:CreateDropdown({
...
-- ============================================================
-- [BLOCK 45] FARM BOSS THEO CHỌN
-- BossDropdown: danh sách boss từ BossList (theo biển)
-- Auto Farm Boss: nhận quest (nếu bật), tìm boss, tấn công
-- Auto Farm All Boss: tìm boss gần nhất trong BossList
-- ============================================================

Tabs.Main:CreateSection({ Text = "Farm Boss", Icon = "rbxassetid://0" })

		BossDropdown = Tabs.Main:CreateDropdown({
		Title = "Select Boss",
		Options = BossList,
		Callback = function(value)
			_G.FindBoss = value
...
FarmBoss = Tabs.Main:CreateToggle({
...
FarmAllBoss = Tabs.Main:CreateToggle({
...
-- ============================================================
-- [BLOCK 46] TAB SETTINGS - CÀI ĐẶT CHUNG
-- RandomCFrame (Spin XYZ): xoay ngẫu nhiên quanh mob
-- Safe Mode: HP < Num_self% -> tween lên Y+500
-- Remove Hit VFX: xóa SlashHit/CurvedRing/SwordSlash
-- Remove Death/Respawn VFX: xóa effect khi chết/hồi
-- Disable Notify: tắt DamageCounter và Notifications
-- Anti AFK: giả click khi idle
-- Anti Admin Join: hop khi admin/dev vào server
-- No Clip: tắt CanCollide trên Stepped
-- ============================================================
...
Tabs.Main:CreateSection({ Text = "Farming Mastery", Icon = "rbxassetid://0" })
local posMastery = {"Cake","Bone"}

local Mastery_Config = Tabs.Main:CreateDropdown({
Title = "Choose Island",
		Options = posMastery,
		Default = Bone,
		Callback = function(Value)
...
local MasteryFruits = Tabs.Main:CreateToggle({
...
local MasteryGun = Tabs.Main:CreateToggle({
...
local MasterySword = Tabs.Main:CreateToggle({
...
local function StartAuraFruit()
    if AuraConnection then
        AuraConnection:Disconnect()
    end

    AuraConnection = RunService.Heartbeat:Connect(function()
        if not getgenv().AuraFruit then
            return
        end
...
Tabs.Settings:CreateSection({ Text = "Settings / Configure", Icon = "rbxassetid://0" })

Initialize = Tabs.Settings:CreateToggle({
Title = "Fast Attack", 
Description = "", 
Default = true,
Callback = function(Value)
...
Tabs.Settings:CreateToggle({
    Title = "M1 Fruits Attack",
    Default = false,
    Callback = function(Value)
        getgenv().AuraFruit = Value
...
Bringmob = Tabs.Settings:CreateToggle({
Title = "Bring Mobs", 
Description = "", 
Default = true,
Callback = function(Value)
...
BusuAura = Tabs.Settings:CreateToggle({
...
RaceV3Aura = Tabs.Settings:CreateToggle({
...
RaceV4Aura = Tabs.Settings:CreateToggle({
...
RandomAround = Tabs.Settings:CreateToggle({
...
SafeModes = Tabs.Settings:CreateToggle({
Title = "Safe Mode", 
Description = "", 
Default = false,
Callback = function(Value)
...
DisableHitVFX = Tabs.Settings:CreateToggle({
...
RmvVFX = Tabs.Settings:CreateToggle({
...
DisblesNotify = Tabs.Settings:CreateToggle({
...
-- ============================================================
-- [BLOCK 47] TAB ESP - AUTO STAT UPGRADE
-- Stats Value slider: số điểm thêm mỗi lần
-- Auto Melee/Sword/Gun/Blox Fruit/Defense: gọi statsSetings()
-- ============================================================

Tabs.Esp:CreateSection({ Text = "Stats Upgrade", Icon = "rbxassetid://0" })

StatusSelect = Tabs.Esp:CreateSlider({
Title = "Stats Value",
Default = 10,
Min = 0,
Max = 1000,
...
StatsUpg = Tabs.Esp:CreateToggle({
Title = "Auto Melee", 
Description = "", 
Default = false,
Callback = function(Value)
...
-- ============================================================
-- [BLOCK 48] TAB FISH - CÂU CÁ TỰ ĐỘNG
-- Select Rod/Bait + Auto Buy Bait (craft mỗi 2s)
-- Auto Fishing:
--   Trang bị cần -> StartCasting -> CastLineAtLocation
--   Khi Biting: Catching -> Catch(1)
-- Auto Quest Fishing: nhận quest câu cá tự động
-- ============================================================

Tabs.Fish:CreateSection({ Text = "Fishing", Icon = "rbxassetid://0" })

Tabs.Fish:CreateDropdown({
...
BaitDropdown = Tabs.Fish:CreateDropdown({
    Title = "Select Bait",
...
BuyBaitToggle = Tabs.Fish:CreateToggle({
    Title = "Auto Buy Bait",
    Default = false,
    Callback = function(Value)
...
FishingToggle = Tabs.Fish:CreateToggle({
    Title = "Auto Fishing",
    Default = false,
    Callback = function(Value)
...
FishingQ = Tabs.Fish:CreateToggle({
...
QuestToggle = Tabs.Fish:CreateToggle({
...
SellFishToggle = Tabs.Fish:CreateToggle({
...
SpamSkillZ = Tabs.Fish:CreateToggle({
...
TravelDress = Tabs.Quests:CreateToggle({
...
Tabs.Quests:CreateSection({ Text = "Tushita + Yama", Icon = "rbxassetid://0" })

Q = Tabs.Quests:CreateToggle({
...
Tabs.Quests:CreateSection({ Text = "Skull Guitars / Misc", Icon = "rbxassetid://0" })

local CheckSoul = Tabs.Quests:CreateParagraph({ Title = "Skull Guitar Quests", Description = "" })
spawn(function()
    while wait(0.2) do
        pcall(function()
            if Quest1 == true then 
                CheckSoul:SetDescription("Quest Number : Quest1")
            elseif Quest2 == true then 
                CheckSoul:SetDescription("Quest Number : Quest2")
            elseif Quest3 == true then 
                CheckSoul:SetDescription("Quest Number : Quest3")
            elseif Quest4 == true then 
                CheckSoul:SetDescription("Quest Number : Quest4")
            elseif GetWP("Skull Guitar") then 
                CheckSoul:SetDescription("Quest Number : Collect!!")
            else 
                CheckSoul:SetDescription("Quest Number : No Quest!!")
            end
        end)
    end
end)
...
Tabs.Quests:CreateSection({ Text = "Cursed Dual Katana", Icon = "rbxassetid://0" })

local CheckCDK = Tabs.Quests:CreateParagraph({ Title = "Number Cursed dual katana quests", Description = "Quest Numbers :" })
spawn(function()  
    while wait(0.2) do 
        if QuestYama_1 == true then 
            CheckCDK:SetDescription("Quest Numbers : yama quest 1") 
        elseif QuestYama_2 == true then
            CheckCDK:SetDescription("Quest Numbers : yama quest 2") 
        elseif QuestYama_3 == true then
            CheckCDK:SetDescription("Quest Numbers : yama quest 3") 
        elseif QuestTushita_1 == true then
            CheckCDK:SetDescription("Quest Numbers : tushita quest 1") 
        elseif QuestTushita_2 == true then
            CheckCDK:SetDescription("Quest Numbers : tushita quest 2") 
        elseif GetWP("Cursed Dual Katana") then
            CheckCDK:SetDescription("Quest Numbers : CDK done!!")
        end 
    end
end)
...
-- ============================================================
-- [BLOCK 49] TAB QUESTS - QUEST & VẬT PHẨM ĐẶC BIỆT
-- TTK: Mua Legendary Sword 1/2/3 -> mua TTK
-- Auto Pole V1: farm Thunder God lấy Pole
-- Auto Pole V2: Pole mastery + awaken Rumble fruit
-- Auto Saw/Saber/Yama/Dark Blade: quest vũ khí W1/W2
-- Auto Tushita CDK: quest CDK thiện (BoatQuest + Cake Queen)
-- Auto Yama CDK: quest CDK ác (Soul Reaper + HellDimension)
-- ============================================================

Tabs.Quests:CreateSection({ Text = "True Triple Katana Sword", Icon = "rbxassetid://0" })
...
Tabs.Quests:CreateSection({ Text = "Pole / God Enal's", Icon = "rbxassetid://0" })
...
Tabs.Quests:CreateSection({ Text = "Rengoku Sword", Icon = "rbxassetid://0" })
...
Tabs.Quests:CreateSection({ Text = "Cavender + Twin Hooks + Bigmom", Icon = "rbxassetid://0" })
...
Tabs.Quests:CreateSection({ Text = "Buso/Aura Colours", Icon = "rbxassetid://0" })
...
Tabs.Quests:CreateSection({ Text = "Instinct / Observation", Icon = "rbxassetid://0" })
...
Bartilo = Tabs.Quests:CreateToggle({
...
CitizenQ = Tabs.Quests:CreateToggle({
...
Tabs.Quests:CreateSection({ Text = "Fighting Melee Styles", Icon = "rbxassetid://0" })

SuperHuman = Tabs.Quests:CreateToggle({
...
DeathStep = Tabs.Quests:CreateToggle({
...
-- ============================================================
-- [BLOCK 50] AUTO COMBAT SKILLS UNLOCK
-- Auto DeathStep: BlackLeg level 400 -> DeathStep
-- Auto SharkmanKarate: FishmanKarate level 400 -> SharkmanKarate
-- Auto ElectricClaw: Electro level 400 -> ElectricClaw
-- Auto DragonTalon: DragonClaw level 400 -> DragonTalon
-- Auto Godhuman: collect 4 nguyên liệu -> BuyGodhuman
-- Auto SanguineArt: Vampire Fang + Demonic Wisp + Dark Fragment
-- ============================================================
  while wait(Sec) do
...
SharkManV2 = Tabs.Quests:CreateToggle({
...
ElectricClaw = Tabs.Quests:CreateToggle({
...
DragonTalon = Tabs.Quests:CreateToggle({
...
Godhuman = Tabs.Quests:CreateToggle({
Title = "Auto Godhuman", 
Description = "", 
Default = false,
Callback = function(Value)
...
SanguineArt = Tabs.Quests:CreateToggle({
...
-- ============================================================
-- [BLOCK 51] TAB RACE - MIRAGE ISLAND & RACE V4
-- FullMoon (0/8->7/8), Mirage Island status
-- Auto Find Mirage: mua thuyền -> ngồi -> tween ra X=-10M
-- Auto Tween To Mirage: TP tới tâm Mirage Island
-- Auto Highest Point: lên đỉnh +400Y
-- Auto Collect Gear: nhặt gear trong MysticIsland
-- ESP Mirage: hiển thị ESP cho island
-- ============================================================

Tabs.Race:CreateSection({ Text = "Mystic Island / Full Moon", Icon = "rbxassetid://0" })

local FullMOOn = Tabs.Race:CreateParagraph({ Title = "FullMoon Status", Description = "" })

local Ismirage = Tabs.Race:CreateParagraph({ Title = "Mirage Island Status", Description = "" })
spawn(function()
    while wait(0.2) do
        if workspace.Map:FindFirstChild("MysticIsland") or workspace._WorldOrigin.Locations:FindFirstChild("Mirage Island") then
            Ismirage:SetDescription("Mirage Island : True")
        else
            Ismirage:SetDescription("Mirage Island : False")
        end
    end
end)
spawn(function()
    while wait(0.2) do
        pcall(function()
            ...
            if moon == moon1 then
                FullMOOn:SetDescription("Moon : 0 / 8")
            elseif moon == moon2 then
                FullMOOn:SetDescription("Moon : 1 / 8")
            elseif moon == moon3 then
                FullMOOn:SetDescription("Moon : 2 / 8")
            elseif moon == moon4 then
                FullMOOn:SetDescription("Moon : 3 / 8 [ Next Night ]")
            elseif moon == moon5 then
                FullMOOn:SetDescription("Moon : 4 / 8 [ Full Moon ]")
            elseif moon == moon6 then
                FullMOOn:SetDescription("Moon : 5 / 8 [ Last Night ]")
            elseif moon == moon7 then
                FullMOOn:SetDescription("Moon : 6 / 8")
            elseif moon == moon8 then
                FullMOOn:SetDescription("Moon : 7 / 8")
            end
        end)
    end
end)
...
Tabs.Race:CreateSection({ Text = "Upgrade Races V2 And V3", Icon = "rbxassetid://0" })

RaceMink = Tabs.Race:CreateToggle({
...
RaceHuman = Tabs.Race:CreateToggle({
...
RaceSky = Tabs.Race:CreateToggle({
...
RaceFish = Tabs.Race:CreateToggle({
...
Tabs.Race:CreateSection({ Text = "Trials Quest V4", Icon = "rbxassetid://0" })

local CheckTier = Tabs.Race:CreateParagraph({ Title = "Tiers V4 Status", Description = "" })
spawn(function()
    pcall(function()
        while wait(0.2) do
            CheckTier:SetDescription("Tiers - V4 : " .. " " .. plr.Data.Race.C.Value)
        end
    end)
end)
...
AutoKill = Tabs.Race:CreateToggle({
...
Tabs.Prehistoric:CreateSection({ Text = "Dojo Quest", Icon = "rbxassetid://0" })

Tabs.Prehistoric:CreateButton({
...
DojoQ = Tabs.Prehistoric:CreateToggle({
...
BlazeEM = Tabs.Prehistoric:CreateToggle({
...
Tabs.Prehistoric:CreateSection({ Text = "Drago Trial", Icon = "rbxassetid://0" })
...
Toggle = Tabs.Prehistoric:CreateToggle({
...
fireflower = Tabs.Prehistoric:CreateToggle({
Title = "Auto Drago (V2)", 
Description = "", 
Default = false,
Callback = function(Value)
...
dragoTpVolcano = Tabs.Prehistoric:CreateToggle({
...
bdrago = Tabs.Prehistoric:CreateToggle({
...
UpTalon = Tabs.Prehistoric:CreateToggle({
...
Tabs.Prehistoric:CreateSection({ Text = "Volcanic Crafting", Icon = "rbxassetid://0" })
...
Tabs.Prehistoric:CreateSection({ Text = "Prehistoric Island", Icon = "rbxassetid://0" })

local Check_Volcano = Tabs.Prehistoric:CreateParagraph({ Title = "Prehistoric Island Status", Description = "" })
spawn(function()
    while wait(0.2) do
        if workspace.Map:FindFirstChild("PrehistoricIsland") or workspace._WorldOrigin.Locations:FindFirstChild("Prehistoric Island") then
            Check_Volcano:SetDescription("Prehistoric Island : True")
        else
            Check_Volcano:SetDescription("Prehistoric Island : False")
        end
    end
end)
...
-- ============================================================
-- [BLOCK 52] TAB PREHISTORIC - VOLCANO EVENT
-- Find Prehistoric Island: tìm đảo bằng thuyền
-- Auto Start Prehistoric: kích hoạt ActivationPrompt Core
-- Auto Patch Prehistoric:
--   Xóa lava Parts, TouchInterest nguy hiểm
--   Farm Lava Golem, tấn công VolcanoRocks khi Glow bật
-- Kill Aura: giết mob bán kính 500
-- Auto Collect Dino Bones/Dragon Eggs
...
Kaura = Tabs.Prehistoric:CreateToggle({
    Title = "Kill Aura",
    Default = false,
    Callback = function(Value)
...
Vocan = Tabs.Prehistoric:CreateToggle({
...
-- ============================================================
-- [BLOCK 53] TAB SEA EVENT - SỰ KIỆN BIỂN
-- Ship Speed Modifier: tăng MaxSpeed VehicleSeat
-- Auto Press W: gửi key W liên tục khi ngồi thuyền
-- No Clip Ship: tắt CanCollide cho tất cả part thuyền
-- Crafting Items: SharkTooth/TerrorJaw/Leviathan gear/Scroll
-- ============================================================

Tabs.SeaEvent:CreateSection({ Text = "Sea Event / Setting Sail", Icon = "rbxassetid://0" })
...
Tabs.SeaEvent:CreateSection({ Text = "Crafting Items", Icon = "rbxassetid://0" })
...
-- Các tính năng Sea Event: Auto Sea Beast, Auto Leviathan,
-- Auto FishBoat, Auto PGB, Sea Zone selector, v.v...
})

Tabs.SeaEvent:CreateSection({ Text = "Choose Sea Event", Icon = "rbxassetid://0" })

Q = Tabs.SeaEvent:CreateDropdown({
    Title = "Select Boats",
	Options = ListSeaBoat,
	Callback = function(Value)
...
Tabs.SeaEvent:CreateSection({ Text = "Entity Sea Event", Icon = "rbxassetid://0" })
...
Tabs.SeaEvent:CreateSection({ Text = "Kitsune Island / Event", Icon = "rbxassetid://0" })

local Check_Kitsu = Tabs.SeaEvent:CreateParagraph({ Title = "Kitsune Island Status", Description = "" })
spawn(function()
    while wait(0.2) do
        if workspace.Map:FindFirstChild("KitsuneIsland") or workspace._WorldOrigin.Locations:FindFirstChild("Kitsune Island") then
            Check_Kitsu:SetDescription("Kitsune Island : True")
        else
            Check_Kitsu:SetDescription("Kitsune Island : False")
        end
    end
end)
...
Tabs.SeaEvent:CreateSection({ Text = "Frozen Dimension Event", Icon = "rbxassetid://0" })

local FloD = Tabs.SeaEvent:CreateParagraph({ Title = "FrozenDimension Status", Description = "" })
spawn(function()
    pcall(function()
        while wait(0.2) do
            if workspace._WorldOrigin.Locations:FindFirstChild('Frozen Dimension') then
                FloD:SetDescription('Frozen Dimension : True')
            else
                FloD:SetDescription('Frozen Dimension : False')
            end
        end
    end)
end)

local SPYING = Tabs.SeaEvent:CreateParagraph({ Title = "Spy Status", Description = "" })
spawn(function()
    while wait(0.2) do
        pcall(function()
            local spycheck = string.match(replicated.Remotes.CommF_:InvokeServer("InfoLeviathan", "1"), "%d+")
            if spycheck then 
                SPYING:SetDescription("Spy Leviathan : " .. tostring(spycheck))
                if tonumber(spycheck) == 5 then
                    SPYING:SetDescription("Spy Leviathan : Already Done!!")
                end
            end
        end)
    end
...
Tabs.Esp:CreateSection({ Text = "Esp", Icon = "rbxassetid://0" })
...
-- ============================================================
-- [BLOCK 54] TAB RAIDS - TRÁI CÂY & DUNGEON RAID
-- getFruitStock(): lấy danh sách fruit đang bán (cập nhật 60s)
-- Auto Random Fruit: mua fruit ngẫu nhiên từ Cousin
-- Auto Drop/Store/Tween/Collect Fruit: quản lý fruit
-- Select Fruit Shop + Auto Buy Fruit Shop: mua fruit cụ thể
-- Get Fruit In Inventory Below 1M: LoadFruit giá rẻ
-- Buy Dungeon Chips [Beli/Devil Fruit]: mua chip dungeon
-- Auto Buy Chip [Beli/Fruit]: tự động mua chip mỗi giây
-- Auto Start Raid: kích hoạt nút summoner dungeon
-- ============================================================

Tabs.Raids:CreateSection({ Text = "Fruits Options", Icon = "rbxassetid://0" })
...
local stockParagraph = Tabs.Raids:CreateParagraph({ Title = "Stock Fruit", Description = "Loading..." })

task.spawn(function()
    while task.wait(60) do
        pcall(function()
            stockParagraph:SetDescription(getFruitStock())
        end)
    end
end)
...
RandomFF = Tabs.Raids:CreateToggle({
...
-- Quản lý fruit: Auto Drop (DropFruits), Auto Store (UpdStFruit),
-- Auto Tween To Fruit, Auto Collect Fruit (collectFruits)

DropF = Tabs.Raids:CreateToggle({
...
StoredF = Tabs.Raids:CreateToggle({
...
BringF = Tabs.Raids:CreateToggle({
...
-- Dungeon/Raid section: Select Chip, Auto Select Dungeon Chip,
-- Buy Chip bằng Beli hoặc Devil Fruit, Auto Start Raid

Tabs.Raids:CreateSection({ Text = "Raiding", Icon = "rbxassetid://0" })


DungeonTables = {"Flame","Ice","Quake","Light","Dark","String","Rumble","Magma","Human: Buddha","Sand","Bird: Phoenix","Dough"}

Q = Tabs.Raids:CreateDropdown({
Title = "Select Chip",
...
Tabs.Raids:CreateButton({
Title = "Buy Raid Chips [Devil Fruit]", 
Callback = function()
...
AutoChipBeli = Tabs.Raids:CreateToggle({
...
AutoChipFruit = Tabs.Raids:CreateToggle({
...
StartR = Tabs.Raids:CreateToggle({
...
Raiding = Tabs.Raids:CreateToggle({
...
Tabs.Raids:CreateSection({ Text = "Items Law/Order Sword", Icon = "rbxassetid://0" })
...
Tabs.Raids:CreateSection({ Text = "Dungeons", Icon = "rbxassetid://0" })

local Players = game:GetService("Players")
local plr = Players.LocalPlayer

_G.Select_Card = "Melee"
_G.Pickcard = false

local D_ClickedCards = {}

local function D_IterCards(cb)
    ...
    for _, g in next, pg:GetChildren() do
        local t = g.Name == "ScreenGui"
            and g:FindFirstChild("1")
            and g["1"]:FindFirstChild("2")

        if t then
            local ok, name = pcall(function()
                return t.DisplayName.Text
            end)
...
local function ResetPickCard()
    table.clear(D_ClickedCards)
end

local function D_PickOneCard()
    local want = _G.Select_Card or "Melee"

    D_IterCards(function(t, name)
        if name == want and not D_ClickedCards[t] then
            D_ClickedCards[t] = true

            pcall(function()
                if firesignal and t.Activated then
                    firesignal(t.Activated)
                elseif t.Activate then
                    t:Activate()
                end
            end)
        end
...
Tabs.Raids:CreateDropdown({
    ...
    Callback = function(selected)
        local value = typeof(selected) == "table"
            and (selected[1] or "Melee")
            or selected

        _G.Select_Card = value
        ResetPickCard()
    end,
})
...
Tabs.Raids:CreateToggle({
    ...
    Description = "Tự động chọn thẻ đã chọn.",
    Default = false,
...
Tabs.Raids:CreateToggle({
    ...
    Description = "Tự động farm dungeon và tự chuyển tầng.",
...
--// Dungeon PlaceId
local DUNGEON_ID = 73902483975735
...
local RANGE = 5000
local OFFSETY = 30
local PRIORITY = "PropHitboxPlaceholder"
local SHADOW = "Blank Buddy"

-- phần code phía dưới giữ nguyênmob này
...
-- Kiểm tra có đang ở dungeon không
local function InDungeon()
    return workspace:FindFirstChild("Map")
        and workspace.Map:FindFirstChild("Dungeon")
end

-- Kiểm tra đúng map dungeon
local function DungeonWorld()
    return game.PlaceId == DUNGEON_ID
end
...
local function Floor()
    ...
    if not hrp or not InDungeon() then
        return
    end
    ...
    for _, floor in ipairs(dungeon:GetChildren()) do
        if floor:IsA("Model") then
            ...
            -- Nếu player đang nằm trong floor này
            if math.abs(pos.X) <= size.X / 2 + 25
                and math.abs(pos.Y) <= size.Y / 2 + 25
                and math.abs(pos.Z) <= size.Z / 2 + 25 then
                return floor
            end
...
-- Kiểm tra mob hợp lệ
local function Valid(mob, hrp)
    if not mob or not mob.Parent then
        return
    end

    if mob.Name == SHADOW then
        return
    end
...
local function BestMob()
    ...
    for _, mob in ipairs(enemies:GetChildren()) do
        ...
        -- Ưu tiên mob đặc biệt
        if mob.Name == PRIORITY then
            return mob
        end

        if Valid(mob, hrp) then
            local dist = (
                hrp.Position -
...
local function Freeze(mob)
    if mob:FindFirstChild("Ngu") then
        return
    end
...
local function NextFloor()
    if _G.GoingExit
        or not _G.Dungeonh
        or not DungeonWorld() then
        return
    end
    ...
    for _ = 1, 8 do
        ...
        if exit then
            _tp(exit.CFrame * CFrame.new(0, 3, 0))
            ...
            repeat
                ...
                if hrp and (hrp.Position - exit.Position).Magnitude < 8 then
                    _G.GoingExit = false
                    return
                end
            until tick() > timeout
...
task.spawn(function()
    ...
    while task.wait(0.05) do
        ...
        if not _G.Dungeonh
            or _G.GoingExit
            or _G.DeathPause then
            continue
        end

        if not DungeonWorld()
            or not InDungeon() then
            continue
        end
        ...
        pcall(function()
            ...
            local mob = lockTarget or BestMob()

            -- Hết mob -> sang tầng mới
            if not mob or not Valid(mob, hrp) then
                lockTarget = nil
                NextFloor()
                return
            end
            ...
            repeat
                ...
                -- Ưu tiên đổi target nếu mob đặc biệt xuất hiện
                if tick() - lastCheck > 0.2 then
                    lastCheck = tick()

                    local newMob = BestMob()

                    if newMob and newMob.Name == PRIORITY then
                        lockTarget = newMob
                        mob = newMob
...
task.spawn(function()
    ...
    while task.wait(0.05) do
        ...
        while _G.Dungeonh
            ...
            task.wait(
                tonumber(_G.Fast_Delay) or 0.06
            )
...
Tabs.Combat:CreateSection({ Text = "Combat / AimBot", Icon = "rbxassetid://0" })

local __indexPlayer = Tabs.Combat:CreateParagraph({ Title = "All Players On Server", Description = "" })
    ...
    while wait(Sec) do
        pcall(function()
            ...
            if playerCount == 12 then
                __indexPlayer:SetDescription("All Players : " .. playerCount .. " / 12 [Max]")
            else
                __indexPlayer:SetDescription("All Players : " .. playerCount .. " / 12")
            end
        end)
    end
end)

local __AimBotTurn = Tabs.Combat:CreateParagraph({ Title = "Aimbot Status", Description = "" })
        ...
        pcall(function()
            if _G.AimMethod then
                if (_G.AimCam or _G.AimbotGun) then
                    __AimBotTurn:SetDescription("Aimbot - " .. Checking_AimStatus() .. " : True")
                else
                    __AimBotTurn:SetDescription("Aimbot - Skills : True")
                end
            else
                __AimBotTurn:SetDescription("Aimbot - Skills : False")
            end
        end)
    end
end)
...
-- ============================================================
-- [BLOCK 55] TAB COMBAT - CHIẾN ĐẤU
-- Player selector + TP to Player/Spectate
-- Aimbot Method (Aim Player/Nearest Aim): redirect MousePos
-- Aimbot Camera: xoay camera về người chơi gần nhất
-- Quest Players: Auto Get/Kill PlayerQuest
-- Auto Enable PvP: kích hoạt PvP liên tục
-- Auto Safe Mode: TP lên Y+1000 khi nguy hiểm
-- ============================================================
        _G.TpPly = Value
        spawn(function()
            pcall(function()
...
Tabs.Combat:CreateSection({ Text = "Quests Players", Icon = "rbxassetid://0" })
...
-- ============================================================
-- [BLOCK 56] LOCAL PLAYER SETTINGS - FLY & COMBAT
-- toggleFly(value):
--   Bật: tạo BodyGyro + BodyVelocity trên Torso
--   Di chuyển theo camera direction + joystick (mobile)
--   Tắt: destroy BodyGyro/Velocity, bật lại CanCollide
-- Dash No Cooldown: setupvalue cooldown Dodge=0
-- Instance Mink V3 INF: clone Agility FX vào HumanoidRootPart
-- Instance Energy INF: giữ Energy.Value cố định
-- Instance Soru INF: reset Soru cooldown liên tục
-- Instance Observation Range INF: VisionRadius = math.huge
-- ============================================================

Tabs.Combat:CreateSection({ Text = "LocalPlayer Settings", Icon = "rbxassetid://0" })
...
-- Các tính năng Combat khác: Observation INF, Speed modifier,
-- Kill Aura (Combat tab), Auto Haki, Farm Mastery v.v...
        if Value then
...
Tabs.Travel:CreateSection({ Text = "Travel - Worlds", Icon = "rbxassetid://0" })
...
Tabs.Travel:CreateSection({ Text = "Travel - Island", Icon = "rbxassetid://0" })
Location = {}
for i,v in pairs(workspace["_WorldOrigin"].Locations:GetChildren()) do  
  table.insert(Location ,v.Name)
end

Travelllll = Tabs.Travel:CreateDropdown({
...
GoIsland = Tabs.Travel:CreateToggle({
Title = "Auto Travel", 
Description = "", 
Default = false,
Callback = function(Value)
...
Tabs.Travel:CreateSection({ Text = "Travel - Portal", Icon = "rbxassetid://0" })
if World1 then
  Location_Portal = {
    "Sky",
    "UnderWater"
...
PortalTP = Tabs.Travel:CreateDropdown({
Title = "Select Portal",
...
Tabs.Travel:CreateSection({ Text = "Travel - NPCs", Icon = "rbxassetid://0" })
for _, v in pairs(replicated.NPCs:GetChildren()) do table.insert(NPCList, v.Name)end

NPCsPos = Tabs.Travel:CreateDropdown({
Title = "Select NPCs",
Options = NPCList,
Callback = function(Value)
  NPClist = Value
...
GoNPCs = Tabs.Travel:CreateToggle({
...
Tabs.Shop:CreateSection({ Text = "Shop Options", Icon = "rbxassetid://0" })
...
Tabs.Shop:CreateSection({ Text = "Fighting - Style", Icon = "rbxassetid://0" })
...
-- Dropdown chọn Melee (Đã chuyển sang UI mới)
MeleeDropdown = Tabs.Shop:CreateDropdown({
            ...
            while _G.AutoBuyMelee do
                ...
                if not data or not data.Pos then
                    MainWindow:Notify({
                        TitleWords = {
                            "Hệ thống ",
                            { Text = "NatAov Hub", Colors = { Color3.fromRGB(255, 80, 80) } },
                        },
                        TextWords = {
                            "Trạng thái: ",
                            { Text = "Không tìm thấy toạ độ cho Melee này ở Sea hiện tại!", Colors = { Color3.fromRGB(255, 100, 100) } },
                        },
                        Duration = 3,
                    })
                ...
                if lp.Character and lp.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if dist > 15 then
                        -- Bật shouldTween để system noclip hoạt động
                        shouldTween = true
                        -- Tween tới NPC
                        local tween = _tp(data.Pos)
                        -- Chờ tới nơi (tối đa 30s)
                        local t0 = tick()
                        repeat
                            task.wait(0.1)
                            if not _G.AutoBuyMelee then break end
                            dist = (hrp.Position - data.Pos.Position).Magnitude
                        until dist <= 15 or tick() - t0 > 30
                    else
                        -- Đã tới nơi, dừng tween và mua
                        shouldTween = false
                        hrp.CFrame = data.Pos
                        ...
                        MainWindow:Notify({
                            TitleWords = {
                                "Hệ thống ",
                                { Text = "NatAov Hub", Colors = { Color3.fromRGB(80, 220, 80) } },
                            },
                            TextWords = {
                                "Đã mua thành công: ",
                                { Text = SelectedMelee, Colors = { Color3.fromRGB(255, 200, 0) } },
                            },
                            Duration = 2,
                        })
...
Tabs.Shop:CreateSection({ Text = "Accessory", Icon = "rbxassetid://0" })
...
Tabs.Shop:CreateSection({ Text = "Weapon World1", Icon = "rbxassetid://0" })
...
Title = "Buy Flintlock", 
-- ============================================================
-- [BLOCK 57] TAB TRAVEL - TELEPORT & SHOP
-- Dropdown đảo/NPC theo biển + nút TP nhanh
-- ============================================================
Callback = function()
...
Tabs.Shop:CreateSection({ Text = "Fragments shop", Icon = "rbxassetid://0" })
...
Tabs.Shop:CreateButton({
    Title        = "Ghoul Race",
    Description  = "Mua và đổi sang Ghoul Race.",
...
Tabs.Shop:CreateButton({
    Title        = "Cyborg Race",
    Description  = "Mua Cyborg Race từ Cyborg Trainer.",
    Icon         = "rbxassetid://88732835297181",
    Confirmation = true,
    Callback     = function()
...
-- ============================================================
-- [BLOCK 58] TAB MISC - TIỆN ÍCH TỔNG HỢP
-- Redeem All Codes: đổi toàn bộ code khuyến mãi
-- Rejoin Server: teleport lại game hiện tại
-- Hop Server: tìm server mới qua Roblox API
-- Hop to Lowest Players: hop sang server ít người nhất
-- Hop to Lowest Pings: hop sang server ping thấp nhất
-- Input Job ID + Teleport [Job ID]: TP sang server cụ thể
-- Copy JobID: copy Job ID vào clipboard
-- ============================================================

Tabs.Misc:CreateSection({ Text = "Server - Function", Icon = "rbxassetid://0" })
...
-- Player GUI & Misc:
-- Open Awakenings Expert, Open Title Selection
-- Disable Chat/Leaderboard GUI
-- Set Pirate/Marine Team
-- Unlock All Portals: requestEntrance tự động theo vị trí
-- Select Haki States + ChangeBusoStage
-- RTX Mode: ColorCorrection + FogEnd=999 + PointLight
-- Fast Mode: đổi material Plastic
-- Low CPU: gọi LowCpu()
-- Increase Boats: MaxSpeed=350 cho thuyền
-- Remove Sky Fog: xóa LightingLayers/SeaTerrorCC/FantasySky
-- Full Bright: Ambient + ColorShift = (1,1,1)
-- Day/Night selector: ClockTime 12 hoặc 0
-- Walk on Water/Ice Walk: điều chỉnh WaterBase-Plane

Tabs.Misc:CreateSection({ Text = "Player Gui / Others", Icon = "rbxassetid://0" })
...
DisbleChat = Tabs.Misc:CreateToggle({
...
DisbleLeaderB = Tabs.Misc:CreateToggle({
...
UnPortal = Tabs.Misc:CreateToggle({
...
Tabs.Misc:CreateSection({ Text = "Graphics / Haki Stats", Icon = "rbxassetid://0" })

HakiSt = {"State 0","State 1","State 2","State 3","State 4","State 5"}

HakiStat = Tabs.Misc:CreateDropdown({
...
-- Rain Fruits (Client): spawn fruit từ rbxassetid local
-- Full Bright, Day/Night toggle, Walk on Water, Ice Walk

Tabs.Misc:CreateSection({ Text = "Configure - God", Icon = "rbxassetid://0" })

Tabs.Misc:CreateButton({
Title = "Rain Fruits ( Old )", 
Callback = function()
...
Tabs.Misc:CreateButton({
    ...
    Callback = function()
        ...
        local objs = game:GetObjects("rbxassetid://96375671161494")
        if not objs[1] then
            warn("Không tải được asset.")
            return
        end

        local root = objs[1]:GetChildren()[1]
        if not root then
            warn("Không tìm thấy root.")
            return
        end
            ...
            if handle then
                ...
                handle.Touched:Connect(function(hit)
                    ...
                    if hum == Humanoid then
                        ...
                        if Humanoid and not Fruit:IsDescendantOf(workspace) then
        ...
        print("Đã spawn Rain Fruits.")
    end,
}) 

briggt1 = Tabs.Misc:CreateToggle({
...
dayornight = Tabs.Misc:CreateToggle({
Title = "Turn on Time", 
Description = "", 
Default = false,
Callback = function(Value)
...
walkWater = Tabs.Misc:CreateToggle({
...
iceWalk = Tabs.Misc:CreateToggle({
...
-- ============================================================
-- [BLOCK 59] FAST ATTACK MODULE
-- FastAttackModule: tấn công cực nhanh qua RE/RegisterAttack + RegisterHit
--   Rate=0.000000002 (gần như instant)
--   GetNearbyTargets: tìm mục tiêu trong bán kính 60
--   GetTargetParts: lấy Head của mỗi target
--   ExecuteFastAttack: Fire RegisterAttack + RegisterHit liên tục
--
-- HitRegistrationModule.Execute:
--   Scan Enemies + Characters trong bán kính 60
--   Chỉ kích hoạt khi tool là Melee hoặc Sword
--   Lấy seed từ server, mã hóa XOR remoteCode
--   Tính finalId = XOR(AttackRemoteId+909090, seed*2)
--   Fire AttackRemoteTarget với encoded params
--
-- InitializeHitRegistration:
--   Scan các folder (Util/Common/Remotes/Assets/FX)
--   Tìm RemoteEvent có Attribute "Id" để intercept
--
-- StartMainLoops:
--   FastAttack: task.spawn loop theo Rate
--   HitRegistration: Heartbeat:Connect (pcall wrapped)
-- ============================================================
...
-- DisableCameraShake: tắt CameraShaker module (không dùng trong StartMainLoops)
...
-- StartMainLoops(): khởi động 2 vòng lặp chính
-- 1. FastAttack loop theo Rate
-- 2. HitRegistration trên Heartbeat
...
-- Gửi notification "NatAov Hub da comeback" khi load xong
StartMainLoops()

-- ==============================================================================
-- 🔄 POLLING LOOP — Quét mỗi 1.5s để bắt Default=true và AutoLoad
-- (Callback bắt click tay tức thì; loop này bắt những trường hợp không có Callback)
-- ==============================================================================
task.delay(3, function()       -- delay nhỏ cho AutoLoad kịp set Value trước
    UpdateDebug()                 -- lần quét đầu sau khi mọi toggle đã được tạo
