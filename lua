...
local function safeWriteFile(name, data)
    ...
    if not ok then
        warn("[spxzz] Save failed: " .. tostring(err))
    end
end
...
pcall(function()
    ...
    if pg then
        local old = pg:FindFirstChild("spxzzGUI")
...
local function enableDarkMode()
    ...
    local sky = Lighting:FindFirstChild("spxzzDarkSky") or Instance.new("Sky")
    sky.Name = "spxzzDarkSky"
...
local function disableDarkMode()
    ...
    local s = Lighting:FindFirstChild("spxzzDarkSky"); if s then s:Destroy() end
...
local function autoSave()
    ...
    task.delay(0.3, function()
        ...
        pcall(function()
            ...
            safeWriteFile("spxzzConfig.json", HttpService:JSONEncode(cfg))
        end)
    end)
end
...
local function loadConfigData()
    if not hasFileFuncs() then return end
    local data = safeReadFile("spxzzConfig.json")
...
local function rebuildQuickButtons()
    ...
    local gui = LP.PlayerGui:FindFirstChild("spxzzGUI")
    ...
    qbBgImg.Image = "https://files.manuscdn.com/user_upload_by_module/session_file/310519663797495683/jBimJBzkWhZlqwvw.png"
    qbBgImg.ScaleType = Enum.ScaleType.Crop
    qbBgImg.ImageTransparency = 0
    qbBgImg.ZIndex = 24
    ...
    btnButtons[1].Activated:Connect(function()
        ...
        _G._spxzzGuiClickActive = true; task.delay(0.1, function() _G._spxzzGuiClickActive = false end)
        State.dropBREnabled = true
        dropBrainrotNow()
...
local function setupHotkeys()
    ...
    UIS.InputBegan:Connect(function(input, gameProcessed)
        ...
        if _G._spxzzBindOverlayActive then return end
        if _G._spxzzGuiClickActive then return end
...
local function rebuildUI()
    ...
    -- spxzz Buyer badge
    local spxzzBuyerBadge = Instance.new("Frame", header)
    spxzzBuyerBadge.Size = UDim2.new(0, 72, 0, 14)
    spxzzBuyerBadge.Position = UDim2.new(0, 58, 0, 27)
    spxzzBuyerBadge.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
    spxzzBuyerBadge.BorderSizePixel = 0
    spxzzBuyerBadge.ZIndex = 6
    Instance.new("UICorner", spxzzBuyerBadge).CornerRadius = UDim.new(0, 4)
    local badgeStroke = Instance.new("UIStroke", spxzzBuyerBadge)
    badgeStroke.Color = Color3.fromRGB(180, 180, 180)
    badgeStroke.Thickness = 1
    local spxzzBuyerLbl = Instance.new("TextLabel", spxzzBuyerBadge)
    spxzzBuyerLbl.Size = UDim2.new(1, 0, 1, 0)
    spxzzBuyerLbl.BackgroundTransparency = 1
    spxzzBuyerLbl.Text = "✦ spxzz Buyer"
    spxzzBuyerLbl.TextColor3 = Color3.fromRGB(80, 215, 230)
    spxzzBuyerLbl.Font = Enum.Font.GothamBold
    spxzzBuyerLbl.TextSize = 8
    spxzzBuyerLbl.TextXAlignment = Enum.TextXAlignment.Center
    spxzzBuyerLbl.ZIndex = 7
    ...
    titleLbl.Text = "◆  spxzz hub DUELS"
    titleLbl.Font = Enum.Font.GothamBlack
    titleLbl.TextSize = 15
    titleLbl.TextXAlignment = Enum.TextXAlignment.Right
    titleLbl.TextColor3 = Color3.fromRGB(80, 215, 230)
    titleLbl.ZIndex = 6
    ...
    sbBox.Focused:Connect(function() _G._spxzzGuiClickActive = true end)
    sbBox.FocusLost:Connect(function() task.delay(0.1, function() _G._spxzzGuiClickActive = false end) end)
    ...
    local function makeToggleRow(n, label, defOn, onToggle, hotkey)
        ...
        clk.Activated:Connect(function()
            ...
            _togDebounce = true
            _G._spxzzGuiClickActive = true
            task.delay(0.3, function() _togDebounce = false end)
            task.delay(0.1, function() _G._spxzzGuiClickActive = false end)
    ...
    do
        ...
        makeGap("Bypass", 5)
        makeSectionLbl("Bypass", "spxzz hub BYPASS")
        -- ── spxzz hub BYPASS BUTTON ──────────────────────────────────────────────
        addCombatPanelBtn("spxzz Bypass", function()
            local panelName = "spxzzBypassPanel"
            ...
            bpTitle.Text = "spxzz SPEED BYPASS"
        ...
        addCombatPanelBtn("Insta Reset", function()
            local panelName = "spxzzInstaResetPanel"
            local existing = gui:FindFirstChild(panelName)
            if existing then existing.Visible = not existing.Visible; return end
        ...
        addCombatPanelBtn("Open Anti", function()
            local panelName = "spxzzAntiPanel"
            local existing = gui:FindFirstChild(panelName)
            if existing then existing.Visible = not existing.Visible; return end
            ...
            apTitle.Text = "spxzz APHEX  .gg/spxzz"
    ...
    do
        ...
        openBtn.Activated:Connect(function()
            ...
            Title2.BackgroundTransparency = 1; Title2.Text = "spxzz duel lagger"
    ...
    overlayGui.Name = "spxzzBindOverlay"; overlayGui.ResetOnSpawn = false
    ...
    local function stopListening()
        ...
        task.delay(0.05, function()
            _G._spxzzBindOverlayActive = false
        end)
    end
    local function startListening(action, inputType, actionLabel, typeLbl)
        ...
        _G._spxzzBindOverlayActive = true
    end
    ...
    makeGap("Binds",5)
    do
        local ft = Instance.new("TextLabel", tabPages["Binds"]); ft.Size = UDim2.new(1,0,0,16); ft.BackgroundTransparency = 1; ft.LayoutOrder = LO("Binds"); ft.Text = "spxzz.cc · v2.3"; ft.TextColor3 = Color3.fromRGB(60, 60, 75); ft.Font = Enum.Font.Gotham; ft.TextSize = 8; ft.TextXAlignment = Enum.TextXAlignment.Center
    end
    switchTab("Steal")
end
...
local introspxzz = Instance.new("TextLabel", introOverlay)
introspxzz.Size = UDim2.new(0.5, -10, 0, 60)
introspxzz.Position = UDim2.new(-1, 0, 0.5, -50)
introspxzz.BackgroundTransparency = 1
introspxzz.Text = "spxzz"
introspxzz.TextColor3 = Color3.fromRGB(80, 215, 230)
introspxzz.Font = Enum.Font.GothamBlack
introspxzz.TextSize = 32
introspxzz.TextXAlignment = Enum.TextXAlignment.Right
introspxzz.TextTransparency = 0
introspxzz.ZIndex = 2
introspxzz.TextStrokeTransparency = 0.6
introspxzz.TextStrokeColor3 = Color3.fromRGB(180, 180, 180)
...
task.spawn(function()
    -- Slide spxzz in from left + zoom out from small
    introspxzz.TextSize = 8
    TweenService:Create(introspxzz, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
    ...
    TweenService:Create(introspxzz, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
...
-- ========== AUTO STEAL PROGRESS WINDOW (spxzz_4 style) ==========
...
-- ========== spxzz USER TAG ==========
-- Mark this client so other spxzz users can detect it
_G._spxzzUserTag = true

local spxzz_TAG_NAME = "spxzzUserTag"
local spxzz_TAG_COLOR = _activeTheme and _activeTheme.accent2 or Color3.fromRGB(180, 180, 200)

local function applyspxzzTag(char)
    if not char then return end
    local head = char:WaitForChild("Head", 5)
    if not head then return end
    local old = head:FindFirstChild(spxzz_TAG_NAME)
    if old then old:Destroy() end

    local bb = Instance.new("BillboardGui", head)
    bb.Name = spxzz_TAG_NAME
...
local function removespxzzTag(char)
    if not char then return end
    local head = char:FindFirstChild("Head")
    if not head then return end
    local tag = head:FindFirstChild(spxzz_TAG_NAME)
    if tag then tag:Destroy() end
end

local function checkAndTagPlayer(player)
    -- Poll _G._spxzzUserTag on the player's client via a shared _G check
    ...
    task.spawn(function()
        ...
        task.wait(1.5)
        if player:GetAttribute("spxzzUser") then
            applyspxzzTag(char)
        end
        -- Also watch for it appearing later
        player:GetAttributeChangedSignal("spxzzUser"):Connect(function()
            if player:GetAttribute("spxzzUser") then
                applyspxzzTag(player.Character)
            else
                removespxzzTag(player.Character)
            end
        end)
        -- Re-apply on respawn
        player.CharacterAdded:Connect(function(newChar)
            task.wait(1.5)
            if player:GetAttribute("spxzzUser") then
                applyspxzzTag(newChar)
            end
        end)
    end)
end

-- Broadcast that this local player is a spxzz user via a replicated Attribute
LP:SetAttribute("spxzzUser", true)
...
task.spawn(function()
    for _ = 1, 10 do
        ...
        if char then
            local head = char:FindFirstChild("Head")
            if head and not head:FindFirstChild(spxzz_TAG_NAME) then
                applyspxzzTag(char)
                break
            end
        end
...
-- Print hotkey guide
print("========================================")
print("[spxzz] LOADED - CUSTOM BINDS ACTIVE")
print("========================================")
