...
do
    ...
    local function shouldPlayIntro()
        ...
        if isfile and isfile("moon_config.json") then
            local ok, cfg = pcall(function() return game:GetService("HttpService"):JSONDecode(readfile("moon_config.json")) end)
    ...
    if shouldPlayIntro() then
        ...
        local BLUE_LIGHT = Color3.fromRGB(100, 180, 255)
        local BLUE_MED = Color3.fromRGB(70, 130, 220)
        local BLUE_DARK = Color3.fromRGB(30, 80, 180)
        ...
        webLabel.Text = "⬡"
        webLabel.TextColor3 = BLUE_LIGHT
        ...
        webLabel2.Text = "⬡"
        webLabel2.TextColor3 = BLUE_LIGHT
        ...
        sp5derText.Text = "MOON"
        sp5derText.TextColor3 = BLUE_MED
        ...
        underline.BackgroundColor3 = BLUE_DARK
...
local function buildGui()
    local BLUE_PRIMARY = Color3.fromRGB(40, 100, 200)
    local BLUE_SECONDARY = Color3.fromRGB(60, 130, 220)
    local BLUE_LIGHT = Color3.fromRGB(100, 180, 255)
    local BLUE_DIM = Color3.fromRGB(20, 50, 120)
    local BLUE_TEXT = Color3.fromRGB(150, 200, 255)
    local BG = Color3.fromRGB(10, 15, 30)
    local CARD = Color3.fromRGB(15, 25, 50)
    local TEXT = Color3.fromRGB(200, 220, 255)
    local BLACK = Color3.fromRGB(5, 8, 20)

    local old = game:GetService("CoreGui"):FindFirstChild("MoonHub")
    if old then old:Destroy() end
    local pg = LP:FindFirstChild("PlayerGui")
    if pg then local o = pg:FindFirstChild("MoonHub"); if o then o:Destroy() end end
    ...
    local function addAnimatedStroke(frame, color, thickness)
        ...
        stroke.Color = color or BLUE_DIM
    ...
    pcall(function()
        if isfile and isfile("moon_config.json") then
            local ok, d = pcall(function() return HS:JSONDecode(readfile("moon_config.json")) end)
    ...
    bubble.Name = "MoonBubble"
    bubble.Size = UDim2.new(0, 100, 0, 26)
    bubble.Position = savedPos("bubblePosXS", "bubblePosXO", "bubblePosYS", "bubblePosYO", UDim2.new(0, 20, 0.5, -13))
    bubble.BackgroundColor3 = BG
    bubble.BorderSizePixel = 0
    bubble.Text = "moon hub"
    bubble.TextColor3 = BLUE_TEXT
    bubble.Font = Enum.Font.Bangers
    bubble.TextSize = 12
    bubble.ZIndex = 20
    local bubbleCorner = Instance.new("UICorner", bubble)
    bubbleCorner.CornerRadius = UDim.new(0, 8)
    addAnimatedStroke(bubble, BLUE_DIM, 2)

    do
        local dn, ds, sp2 = false
        ...
        UIS.InputChanged:Connect(function(i)
            if dn and i.UserInputType == Enum.UserInputType.MouseMovement then
                ...
                local gv = getgenv and getgenv(); if gv then gv._moon_bubblePos = bubble.Position end
            end
        end)
    end
    ...
    main.Name = "Main"
    main.Size = UDim2.new(0, math.floor(260 * guiScale), 0, math.floor(340 * guiScale))
    main.Position = savedPos("mainPosXS", "mainPosXO", "mainPosYS", "mainPosYO", UDim2.new(0.5, -130, 0.5, -170))
    ...
    addAnimatedStroke(main, BLUE_DIM, 2.5)
    _mainFrame = main
    ...
    local function dragFrame(frame, posKey)
        ...
        UIS.InputChanged:Connect(function(i)
            if dn and not guiLocked and i.UserInputType == Enum.UserInputType.MouseMovement then
                local delta = i.Position - ds
    ...
    dragFrame(main, "_moon_mainPos")
    local gv0 = getgenv and getgenv()
    if gv0 then gv0._moon_mainPos = main.Position; gv0._moon_bubblePos = bubble.Position end
    ...
    closeBtn.Text = "X"
    closeBtn.TextColor3 = BLUE_TEXT
    closeBtn.Font = Enum.Font.Bangers
    closeBtn.TextSize = 12
    local closeCorner = Instance.new("UICorner", closeBtn)
    closeCorner.CornerRadius = UDim.new(0, 6)
    addAnimatedStroke(closeBtn, BLUE_DIM, 1)
    closeBtn.MouseEnter:Connect(function()
        TS:Create(closeBtn, TweenInfo.new(0.1), {BackgroundColor3 = BLUE_DIM, TextColor3 = BLUE_LIGHT}):Play()
    end)
    closeBtn.MouseLeave:Connect(function()
        TS:Create(closeBtn, TweenInfo.new(0.1), {BackgroundColor3 = CARD, TextColor3 = BLUE_TEXT}):Play()
    end)
    ...
    content.Size = UDim2.new(1, -103, 1, -49)
    content.Position = UDim2.new(0, 93, 0, 46)
    ...
    local function createTabButton(name, order)
        ...
        addAnimatedStroke(btn, BLUE_DIM, 1.2)
        return btn
    end
    ...
    local function switchTab(tabName)
        ...
        for name, btn in pairs(tabs) do
            if name == tabName then
                btn.BackgroundColor3 = BLUE_LIGHT
    ...
    local function mkRow(parent, h)
        ...
        addAnimatedStroke(f, BLUE_DIM, 1)
        f.MouseEnter:Connect(function()
            TS:Create(f, TweenInfo.new(0.08), {BackgroundColor3 = Color3.fromRGB(25, 35, 60)}):Play()
        end)
    ...
    local function animPill(pill, dot, on)
        TS:Create(pill, TweenInfo.new(0.18, Enum.EasingStyle.Quad), {BackgroundColor3 = on and BLUE_DIM or CARD}):Play()
        TS:Create(dot, TweenInfo.new(0.18, Enum.EasingStyle.Back), {
            Position = on and UDim2.new(1, -14, 0.5, -5) or UDim2.new(0, 3, 0.5, -5),
            BackgroundColor3 = on and BLUE_LIGHT or Color3.fromRGB(40, 60, 100),
        }):Play()
    end
    ...
    local function mkPill(row, offset)
        ...
        pill.Position = UDim2.new(1, -(offset or 42), 0.5, -8)
        pill.BackgroundColor3 = CARD
        ...
        dot.BackgroundColor3 = Color3.fromRGB(40, 60, 100)
    ...
    local function mkBox(parent, default, w, xOff, cb)
        ...
        tb.Size = UDim2.new(0, w or 40, 0, 18)
        tb.Position = UDim2.new(1, -(xOff or 50), 0.5, -9)
        tb.BackgroundColor3 = Color3.fromRGB(12, 18, 35)
        tb.BorderSizePixel = 0
        tb.Text = tostring(default)
        tb.TextColor3 = BLUE_TEXT
        ...
        tb.Focused:Connect(function()
            TS:Create(bs, TweenInfo.new(0.12), {Color = BLUE_LIGHT}):Play()
        end)
        tb.FocusLost:Connect(function()
            TS:Create(bs, TweenInfo.new(0.12), {Color = BLUE_DIM}):Play()
            if cb then
    ...
    local function mkKB(parent, kbEntry, cb)
        ...
        btn.BackgroundColor3 = Color3.fromRGB(12, 18, 35)
        btn.BorderSizePixel = 0
    ...
    local function mkToggleKB(parent, txt, kbEntry, onToggle, onKB)
        ...
        local pill, dot = mkPill(row, kbEntry and 100 or 42)
        local on = false
        local function sv(s)
            on = s
            animPill(pill, dot, s)
    ...
    pbFrame.Size = UDim2.new(0, 220, 0, 40)
    pbFrame.Position = savedPos("pbPosXS", "pbPosXO", "pbPosYS", "pbPosYO", UDim2.new(0.5, -110, 1, -55))
    pbFrame.BackgroundColor3 = Color3.fromRGB(10, 15, 30)
    pbFrame.BorderSizePixel = 0
    pbFrame.Active = true
    local pbCorner = Instance.new("UICorner", pbFrame)
    pbCorner.CornerRadius = UDim.new(0, 8)
    addAnimatedStroke(pbFrame, BLUE_DIM, 1.5)
    ...
    do
        ...
        UIS.InputChanged:Connect(function(i)
            if dn and not guiLocked and i.UserInputType == Enum.UserInputType.MouseMovement then
                local delta = i.Position - ds
                pbFrame.Position = UDim2.new(sp.X.Scale, sp.X.Offset + delta.X, sp.Y.Scale, sp.Y.Offset + delta.Y)
                local gv = getgenv and getgenv()
                if gv then gv._moon_pbPos = pbFrame.Position end
            end
        end)
    end
    local gv1 = getgenv and getgenv()
    if gv1 then gv1._moon_pbPos = pbFrame.Position end

    progressPct = Instance.new("TextLabel", pbFrame)
    progressPct.Size = UDim2.new(0, 36, 0, 12)
    progressPct.Position = UDim2.new(0, 6, 0, 5)
    ...
    progressRadLbl.Size = UDim2.new(0, 90, 0, 12)
    progressRadLbl.Position = UDim2.new(1, -96, 0, 5)
    ...
    pbg.Size = UDim2.new(1, -14, 0, 8)
    pbg.Position = UDim2.new(0, 6, 0, 24)
    pbg.BackgroundColor3 = Color3.fromRGB(6, 10, 20)
    pbg.BorderSizePixel = 0
    local pbgCorner = Instance.new("UICorner", pbg)
    pbgCorner.CornerRadius = UDim.new(1, 0)
    progressFill = Instance.new("Frame", pbg)
    progressFill.Size = UDim2.new(0, 0, 1, 0)
    progressFill.BackgroundColor3 = BLUE_LIGHT
    progressFill.BorderSizePixel = 0
    local fillCorner = Instance.new("UICorner", progressFill)
    fillCorner.CornerRadius = UDim.new(1, 0)

    do
        local abRow = mkRow(combatTab, 28)
        ...
        local abPill, abDot = mkPill(abRow, 100)
        abPill.ZIndex = 3
        abDot.ZIndex = 4
        local abOn = false
    ...
    do
        local ab2Row = mkRow(combatTab, 28)
        ...
        local ab2Pill, ab2Dot = mkPill(ab2Row, 100)
        ab2Pill.ZIndex = 3
        ab2Dot.ZIndex = 4
        local ab2On = false
    ...
    do
        local row = mkRow(combatTab, 28)
        mkLabel(row, "Steal Radius")
        radInput = mkBox(row, Steal.StealRadius, 40, 52, function(v)
    ...
    do
        local stealRow = mkRow(combatTab, 28)
    ...
    do
        local row = mkRow(playerTab, 28)
        mkLabel(row, "Normal Speed")
        normalBox = mkBox(row, NS, 40, 48, function(v)
    ...
    do
        ...
        carryBox = mkBox(row, CS, 40, 48, function(v)
            if v > 0 and v <= 500 then
                CS = v
            end
            saveConfig()
    ...
    do
        ...
        laggerBox = mkBox(row, LAGGER_SPEED, 40, 48, function(v)
            if v > 0 and v <= 500 then
                LAGGER_SPEED = v
            end
            saveConfig()
    ...
    do
        ...
        laggerCarryBox = mkBox(row, LAGGER_CARRY_SPEED, 40, 48, function(v)
            if v > 0 and v <= 500 then
    ...
    do
        ...
        modeValLbl.TextColor3 = BLUE_TEXT
    ...
    do
        ...
        autoTPHeightBox = mkBox(row, autoTPHeight, 40, 48, function(v)
            if v >= 0 and v <= 500 then
                autoTPHeight = v
            else
    ...
    creditFrame.Size = UDim2.new(1, 0, 0, 22)
    ...
    creditText.TextColor3 = BLUE_LIGHT
    ...
    do
        ...
        secRow.Size = UDim2.new(1, 0, 0, 18)
    ...
    do
        local row = mkRow(settingsTab, 28)
        mkLabel(row, "Main GUI Scale")
        local box = mkBox(row, guiScale, 40, 48, function(v)
            applyGuiScale(v)
            saveConfig()
        end)
        box.Text = tostring(guiScale)
        local hint = Instance.new("TextLabel", row)
        hint.Size = UDim2.new(0, 55, 1, 0)
        hint.Position = UDim2.new(1, -105, 0, 0)
        hint.BackgroundTransparency = 1
        hint.Text = "(0.3-3.0)"
        hint.TextColor3 = BLUE_DIM
    ...
    do
        ...
        local box = mkBox(row, mbScale, 40, 48, function(v)
            applyMbScale(v)
            saveConfig()
        end)
    ...
    local resetRow = mkRow(settingsTab, 28)
    mkLabel(resetRow, "Reset Sizes")
    local resetBtn = Instance.new("TextButton", resetRow)
    resetBtn.Size = UDim2.new(0, 55, 0, 18)
    resetBtn.Position = UDim2.new(1, -61, 0.5, -9)
    resetBtn.BackgroundColor3 = Color3.fromRGB(20, 30, 55)
    resetBtn.BorderSizePixel = 0
    resetBtn.Text = "RESET"
    resetBtn.TextColor3 = BLUE_TEXT
    resetBtn.Font = Enum.Font.Bangers
    resetBtn.TextSize = 8
    local resetCorner = Instance.new("UICorner", resetBtn)
    resetCorner.CornerRadius = UDim.new(0, 6)
    addAnimatedStroke(resetBtn, BLUE_DIM, 1)
    ...
    local lockRow = mkRow(settingsTab, 28)
    mkLabel(lockRow, "Lock UI")
    local lockPill, lockDot = mkPill(lockRow, 42)
    ...
    credRow.Size = UDim2.new(1, 0, 0, 22)
    ...
    credLbl.TextColor3 = BLUE_LIGHT
    credLbl.Font = Enum.Font.Bangers
    credLbl.TextSize = 8
    credLbl.TextXAlignment = Enum.TextXAlignment.Center

    local MB_BTN_SIZE = math.floor(50 * mbScale)
    local MB_PAD = math.floor(7 * mbScale)
    local MB_COLS = 2
    local MB_ROWS = 6
    local MBTOTALW = MB_COLS * MB_BTN_SIZE + (MB_COLS + 1) * MB_PAD
    local MBTOTALH = MB_ROWS * MB_BTN_SIZE + (MB_ROWS + 1) * MB_PAD
    local OUTLINE_COLOR = BLUE_DIM
    local OUTLINE_HOVER = BLUE_LIGHT
    local MB_TEXT_CLR = TEXT
    local MB_ACTIVE = Color3.fromRGB(50, 80, 150)
    local MB_NORMAL = Color3.fromRGB(10, 15, 30)
    local MB_FLASH = Color3.fromRGB(100, 150, 220)

    local oldMb = game:GetService("CoreGui"):FindFirstChild("MoonMobile")
    if oldMb then oldMb:Destroy() end
    local mbPg = LP:FindFirstChild("PlayerGui")
    if mbPg then
        local o = mbPg:FindFirstChild("MoonMobile")
        if o then o:Destroy() end
    end

    local mbGui = Instance.new("ScreenGui")
    mbGui.Name = "MoonMobile"
    ...
    mbPanel.Position = savedPos("mbPosXS", "mbPosXO", "mbPosYS", "mbPosYO", UDim2.new(1, -(MBTOTALW + 12), 0.5, -(MBTOTALH / 2)))
    ...
    if gv2 then gv2._moon_mbPos = mbPanel.Position end
    ...
    do
        ...
        UIS.InputChanged:Connect(function(i)
            if dn and not guiLocked and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then
                local delta = i.Position - ds
                mbPanel.Position = UDim2.new(sp.X.Scale, sp.X.Offset + delta.X, sp.Y.Scale, sp.Y.Offset + delta.Y)
                local gv = getgenv and getgenv()
                if gv then gv._moon_mbPos = mbPanel.Position end
            end
        end)
    end

    local function makeDraggableMobileBtn(label, row, col)
        ...
        btn.InputChanged:Connect(function(i)
            if i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch then
                if btnDn and not guiLocked then
                    local delta = i.Position - btnDs
                    btn.Position = UDim2.new(btnSp.X.Scale, btnSp.X.Offset + delta.X, btnSp.Y.Scale, btnSp.Y.Offset + delta.Y)
                end
            end
        end)
        ...
        btn.MouseButton1Down:Connect(function()
            if btn.BackgroundColor3 == MB_NORMAL then
                TS:Create(btn, TweenInfo.new(0.05), {BackgroundColor3 = Color3.fromRGB(15, 25, 45)}):Play()
            end
        end)
    ...
    local aimbot2Btn = makeDraggableMobileBtn("AIMBOT\n2", 1, 1)
    local dropBtn = makeDraggableMobileBtn("DROP\nBR", 1, 2)
    local autoLeftBtn = makeDraggableMobileBtn("AUTO\nLEFT", 2, 1)
    local autoBatBtn = makeDraggableMobileBtn("AUTO\nBAT", 2, 2)
    local autoRightBtn = makeDraggableMobileBtn("AUTO\nRIGHT", 3, 1)
    local tpDownBtn = makeDraggableMobileBtn("TP\nDOWN", 3, 2)
    local laggerCarryBtn = makeDraggableMobileBtn("LAGGER\nCARRY", 4, 1)
    local carrySpeedBtn = makeDraggableMobileBtn("CARRY\nSPD", 4, 2)
    local laggerModeBtn = makeDraggableMobileBtn("LAGGER\nMODE", 5, 1)
    local instaResetBtn = makeDraggableMobileBtn("INSTA\nRESET", 5, 2)
    ...
    setCarrySpeedVisual = function(s)
        setToggleBG(carrySpeedBtn, s)
    end

    setLaggerCarryVisual = function(s)
        setToggleBG(laggerCarryBtn, s)
    end
    ...
    laggerModeBtn.MouseButton1Click:Connect(function()
        ...
        if setLaggerModeTabVisual then
            setLaggerModeTabVisual(laggerToggled and laggerPhase == 1)
        end
        refreshSpeedModeLabel()
        saveConfig()
    end)
...
local function saveConfig()
    ...
    local gv = getgenv and getgenv()
    if gv then
        if gv._moon_mainPos then
            cfg.mainPosXS = gv._moon_mainPos.X.Scale
            cfg.mainPosXO = gv._moon_mainPos.X.Offset
            cfg.mainPosYS = gv._moon_mainPos.Y.Scale
            cfg.mainPosYO = gv._moon_mainPos.Y.Offset
        end
        if gv._moon_bubblePos then
            cfg.bubblePosXS = gv._moon_bubblePos.X.Scale
            cfg.bubblePosXO = gv._moon_bubblePos.X.Offset
            cfg.bubblePosYS = gv._moon_bubblePos.Y.Scale
            cfg.bubblePosYO = gv._moon_bubblePos.Y.Offset
        end
        if gv._moon_pbPos then
            cfg.pbPosXS = gv._moon_pbPos.X.Scale
            cfg.pbPosXO = gv._moon_pbPos.X.Offset
            cfg.pbPosYS = gv._moon_pbPos.Y.Scale
            cfg.pbPosYO = gv._moon_pbPos.Y.Offset
        end
        if gv._moon_mbPos then
            cfg.mbPosXS = gv._moon_mbPos.X.Scale
            cfg.mbPosXO = gv._moon_mbPos.X.Offset
            cfg.mbPosYS = gv._moon_mbPos.Y.Scale
            cfg.mbPosYO = gv._moon_mbPos.Y.Offset
        end
    end
    if writefile then
        pcall(function()
            writefile("moon_config.json", HS:JSONEncode(cfg))
        end)
    end
end
task.spawn(function()
...
local function loadConfigKeys()
    if not (isfile and isfile("moon_config.json")) then return end
    local ok, cfg = pcall(function() return HS:JSONDecode(readfile("moon_config.json")) end)
